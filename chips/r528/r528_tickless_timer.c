/****************************************************************************
 * chip/r528/r528_tickless_timer.c
 *
...
struct oneshot_lowerhalf_s *r528_timer_initialize(unsigned int freq)
{
  struct arm_timer_lowerhalf_s *lower;
  uint32_t ctrl;
...
{
    unsigned int freq=24000000;
#ifdef CONFIG_DRIVERS_CCMU
    hal_clock_init();
#endif

    FAR struct oneshot_lowerhalf_s *lower = r528_timer_initialize(freq);
