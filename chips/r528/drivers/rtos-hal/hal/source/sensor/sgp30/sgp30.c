                           ...
                           unsigned int num_params)
{
  ...
  sninfo("cmd: 0x%04X num_params: %d ret: %d\n",
                          ...
                          unsigned int num_words)
{
  ...
  sninfo("cmd: 0x%04X delay: %uus ret: %d\n", cmd.address,
  ...
  sninfo("read cmd: 0x%04X num_params: %d ret: %d\n",
                              ...
                              FAR struct sgp30_conv_data_s *data)
{
  ...
  if (ret < 0)
    {
      snerr("ERROR: sgp30_write_cmd failed: %d\n", ret);
      return ret;
    }

  if (sgp30_check_data_crc(words, 2) < 0)
    {
      snerr("ERROR: sgp30_read_words crc failed\n");
      ret = -EIO;
      return ret;
    }
  ...
  if (priv->crefs == 1)
    {
      ...
      if (sgp30_read_cmd(priv, SGP30_CMD_GET_SERIAL_ID, serial, 3) >= 0
          ...
          && sgp30_check_data_crc(buf, 1) >= 0)
        {
          ...
          if (ret < 0)
            {
              snerr("sgp30_write_cmd(SGP30_CMD_INIT_AIR_QUALITY)"
                         " failed, %d\n", ret);
            }
          else
            {
              ...
              sninfo("sgp30_write_cmd(SGP30_CMD_INIT_AIR_QUALITY)\n");
              while (repeat-- &&
                     time_has_passed_ms(&curr, &start, SGP30_INIT_LIMIT_MS))
                {
                  ...
                  if (ret < 0)
                    {
                      snerr("sgp30_soft_reset failed, %d\n", ret);
                      return ret;
                    }
      ...
      if (ret < 0)
        {
          snerr("sgp30 open error\n");
          ret = -ENODEV;
          priv->crefs--;
        }
    }
  sninfo("sgp30 open success\n");
