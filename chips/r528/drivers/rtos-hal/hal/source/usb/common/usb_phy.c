...
static int usb_phy_range_register_write(unsigned long phy_vbase, int data)
{
	...
	_info("phy_range config success!\n");
	return 0;
}
...
static int usb_phy_range_vcbus_write(unsigned long phy_vbase, int data)
{
	...
	case USB_PHY_VERF_MODE:
		...
		if (value != usb_phy_tp_read(phy_vbase, USB_PHY_TRAN_IREF_RISE, len)){
			_err("cann't config trancevie rise range in VERF mode! origin:0x%x, new:0x%lx\n",
	...
	_info("phy_range config in %s mode %s!\n", phy_mode ? "iref" : "vref",
	       ret == 0 ? "success" : "fail");
	return ret;
}
...
static int usb_phy_range_vcbus_read(unsigned long phy_vbase)
{
	...
	_info("phy_range = 0x%lx, details:\n"\
	       "bit[12]   = 0x%lx, mode iref-1 vref-0\n"\
	       "bit[11:9] = 0x%d, rise time(all usb)\n"\
	       "bit[8:6]  = 0x%d, rise time(current usb)\n"\
	       "bit[5:4]  = 0x%d, pre-emphasis\n"\
	       "bit[3:0]  = 0x%d, resistance\n\n",
...
int usb_phy_range_show(unsigned long phy_vbase)
{
...
int usb_phy_range_set(unsigned long phy_vbase, int data)
{
#if USB_PHY_USE_VCBUS
...
void usb_phy_init(unsigned long phy_vbase, unsigned int usbc_no)
{
	...
	_info("phy_vbase : 0x%lx, usbc_no : %x, efuse : 0x%x\n", phy_vbase, usbc_no, efuse);
}
...
void usb_phy_init(unsigned long phy_vbase, unsigned int usbc_no)
{
	...
	_info("calibration finish, val:0x%x, usbc_no:%d\n",
		     usb_phy_tp_read(phy_vbase, 0x1a, 6), usbc_no);
}
#else
void usb_phy_init(unsigned long phy_vbase, unsigned int usbc_no)
{
	_info("using default phy\n");
}
#endif
