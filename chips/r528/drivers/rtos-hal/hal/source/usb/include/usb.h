...
{
	if (!intf) {
		_err("PANIC : usb_get_intfdata() inft == NULL\n");
		return NULL;
	}
	...
	if (intf->virt_sub_dev && intf->virt_sub_dev->drv_pirv_data) {
		...
		_err("PANIC : usb_get_intfdata() intf->virt_sub_dev->drv_pirv_data == NULL\n");
		return NULL;
	}
...
{
	if (!intf) {
		_err("PANIC : interface_to_usbdev :intf == NULL\n");
		return NULL;
	}

	if (!intf->virt_sub_dev) {
		_err("PANIC : interface_to_usbdev :virt_sub_dev == NULL\n");
		return NULL;
	}
