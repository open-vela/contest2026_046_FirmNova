...
static void sunxi_set_acqiure_time(unsigned long reg_base, uint32_t val)
{
	...
	TPADC_INFO("sunxi_set_acqiure_time: 0x%08x\n", reg_val);
...
static void sunxi_set_frequency_divider(unsigned long reg_base, uint32_t val)
{
	...
	TPADC_INFO("sunxi_set_frequency_divider: 0x%08x\n", reg_val);
...
static void sunxi_set_clk_divider(unsigned long reg_base, uint32_t val)
{
	...
	TPADC_INFO("sunxi_set_clk_divider: 0x%08x\n", reg_val);
...
{
	sunxi_set_acqiure_time(reg_base, 100); // 适度增加采集时间
	sunxi_set_frequency_divider(reg_base, 0x8); // 增加频率分频
	sunxi_set_clk_divider(reg_base, 0x2); // 增加时钟分频【0-3】
...
unused_code  static u32 sunxi_tpadc_ch_select(unsigned long reg_base, tp_channel_id id)
{
	u32 reg_val;
...
unused_code static int sunxi_tpadc_ch_deselect(unsigned long reg_base, tp_channel_id id)
{
	u32 reg_val;
...
static hal_irqreturn_t tpadc_handler(void *dev)
{
	...
	if (reg_val & TP_DATAPEND) {
		...
		tpadc->callback((uint16_t)x_data, (uint16_t)y_data, DATA_MOVE);
	}

	else if (reg_val & TP_UPPEND) {
		tpadc->callback(0,0, DATA_UP);
	}

	else if (reg_val & TP_DOWNPEND) {
		tpadc->callback(0,0, DATA_DOWN);
	}
...
hal_tpadc_status_t hal_tpadc_adc_init(void)
{
    ...
    sunxi_downirq_enable(tpadc->reg_base);
    sunxi_upirq_enable(tpadc->reg_base);
    sunxi_set_trig_level(tpadc->reg_base, 0x3); // 从0x1改为0x3，降低中断频率
    sunxi_irq_enable(tpadc->reg_base);
