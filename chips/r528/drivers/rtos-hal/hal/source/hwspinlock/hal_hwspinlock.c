...
void hal_hwspinlock_irq_enable(int num)
{
	writel(readl(SPINLOCK_IRQ_EN_REG) | (1 << num), SPINLOCK_IRQ_EN_REG);
}

void hal_hwspinlock_irq_disable(int num)
{
	writel(readl(SPINLOCK_IRQ_EN_REG) & (~(1 << num)), SPINLOCK_IRQ_EN_REG);
}

void hal_hwspinlock_irq_clear_pending(int num)
{
	writel(1 << num, SPINLOCK_IRQ_STA_REG);
}
