...
typedef struct {
	...
	uint8_t *key;
	uint32_t key_length;
	__attribute__((aligned(CACHELINE_LEN))) uint8_t padding[DES_BLOCK_SIZE];
...
typedef struct {
	...
	uint8_t *key;
	uint32_t key_length;
	__attribute__((aligned(CACHELINE_LEN))) uint8_t padding[AES_BLOCK_SIZE];
	uint32_t padding_len;
	uint32_t dir;
	uint32_t mode;
	uint32_t bitwidth;
...
typedef struct {
	...
	uint8_t *dst_buffer;
	uint32_t dst_length;
	__attribute__((aligned(CACHELINE_LEN))) uint8_t md[SHA_MAX_DIGEST_SIZE];
	uint32_t md_size;
	__attribute__((aligned(CACHELINE_LEN))) uint8_t padding[SHA512_BLOCK_SIZE * 2];
	uint32_t padding_len;
	uint32_t type;
	uint32_t dir;
