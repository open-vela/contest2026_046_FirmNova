...
#define CONF_KERNEL_VERSION_5_4 1
...
#define DISP_DBG_LOG	 if(DISP_LOG)  _info
...
#ifdef __SUNXI_DISPLAY2__
static int LayerConfig(int fd, enum tag_DISP_CMD cmd, struct disp_layer_config *pinfo)
{
	//DISP_DBG_LOG("(%s %d), pinfo channel = %d, pinfo layer_id = %d, pinfo = 0x%08x\n", __FUNCTION__, __LINE__, pinfo->channel, pinfo->layer_id, pinfo);
...
static int LayerGetPara(int disp_fd, struct disp_layer_config *pinfo)
{
	//DISP_DBG_LOG("(%s %d), zorder= %d\n", __FUNCTION__, __LINE__, pinfo->info.zorder);
	return LayerConfig(disp_fd, DISP_LAYER_GET_CONFIG, pinfo);
}

static int LayerSetPara(int disp_fd, struct disp_layer_config *pinfo)
{
...
static int layer_set_src(int fd, unsigned int hlay, SrcInfo *src, renderBuf *picture)
{
	...
	* otherwise the set buffer address will be cleared */
    ...
    DISP_DBG_LOG("lbc_mode = %d, width = %d, height = %d", src->lbc_mod, src->w, src->h);
    if (src->lbc_mod > 0)
    {
...
static int layer_set_zorder(int fd, unsigned int hlay, int zorder)
{
	...
	DISP_DBG_LOG("(%s %d), zorder= %d\n", __FUNCTION__, __LINE__, layer_zorder);
...
int layer_set_colorkey(int fd, unsigned int hlay, unsigned int color)
{
	...
	disp_ioctl(DISP_CMD_SET_COLORKEY,(void*)args);
	if(RET_OK != ret) {
...
int disp_layer_on(int dispFd, unsigned int dispLayer)
{
	...
	ret = disp_ioctl(DISP_LAYER_ENABLE, (void*)ioctlParam);
#else
...
int disp_layer_off(int dispFd, unsigned int dispLayer)
{
	...
	ioctlParam[1] =  HD2CHN(dispLayer);
		...
		ret = disp_ioctl(DISP_LAYER_DISABLE, (void*)ioctlParam);
#else
		...
		case VIDEO_SRC_FROM_FILE:
			if ((vout_hd->rotate == ROTATION_ANGLE_90)
				...
				if (param->srcInfo.format == VIDEO_PIXEL_FORMAT_YUYV) {
					...
					_err("YUYV rotation does not support\n");
				} else {
...
int DispSetZorder(void *hdl, int zorder)
{
	...
	DISP_DBG_LOG("(%s %d), zorder= %d, config.info.zorder = %d\n", __FUNCTION__, __LINE__, layer_zorder, config.info.zorder);
	return LayerSetPara(vout_hd->disp_fd, &config);
#else
#ifdef SUNXI_DISPLAY_GPU
#ifdef SUNXI_DISPLAY_GPU_BUF
...
int DispGetScreenWidth(void *hdl)
{
	...
	width = disp_ioctl(DISP_GET_SCN_WIDTH, ioctlParam);
#else
...
int DispGetScreenHeight(void *hdl)
{
	...
	height = disp_ioctl(DISP_GET_SCN_HEIGHT, ioctlParam);
#else
	DISP_CLAR(ioctlParam);
	ioctlParam[0] = SCREEN_0;
	ioctlParam[1] = 0;
...
int DispSetIoctl(void *hdl, unsigned int cmd,  unsigned long para)
{
	...
	struct disp_layer_config *config = (struct disp_layer_config *)para;
	config->channel = HD2CHN(vout_hd->hlayer);
	config->layer_id = HD2LYL(vout_hd->hlayer);
	ioctlParam[1] = para;
	ioctlParam[2] = 1;
    ...
    ret = disp_ioctl(cmd, (void*) ioctlParam);
#else
...
errHdl:
	...
	ioctlParam[1] = 0;
	//width = ioctl(vout_hd->disp_fd, DISP_CMD_GET_SCN_WIDTH, ioctlParam);
	width = disp_ioctl(DISP_CMD_GET_SCN_WIDTH, ioctlParam);
	//height = ioctl(vout_hd->disp_fd, DISP_CMD_GET_SCN_HEIGHT, ioctlParam);
	height = disp_ioctl(DISP_CMD_GET_SCN_HEIGHT, ioctlParam);
	memset(&config, 0, sizeof(disp_layer_info));
...
int DispResize(void *hdl, __disp_fb_t *input_fb, __disp_fb_t *output_fb)
{
	...
	hdle = disp_ioctl(DISP_CMD_SCALER_REQUEST, args);
	...
	disp_ioctl(DISP_CMD_SCALER_EXECUTE, args);
	...
	hdle = disp_ioctl(DISP_CMD_SCALER_RELEASE, args);
	DISP_DBG_LOG("(%s %d)\n", __FUNCTION__, __LINE__);
...
int DispSetCache(void *hdl,int enable)
{
	...
	disp_ioctl(DISP_SHADOW_PROTECT, ioctlParam);
#else
	//ioctl(vout_hd->disp_fd, DISP_CMD_SHADOW_PROTECT, ioctlParam);
	disp_ioctl(DISP_CMD_SHADOW_PROTECT, ioctlParam);
#endif
	return 0;
}
