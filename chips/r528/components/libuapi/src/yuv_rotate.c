...
#define  __SUNXI_DISPLAY2__  0
#if defined(__SUNXI_DISPLAY2__) && (defined(CONFIG_RISCV))
...
{
#define ROTAGE_90_NV21 0
#define ROTAGE_90_YU12 1
	...
	unsigned int format = ROTAGE_90_NV21;
	unsigned int row_bytes = width;
	unsigned int col_bytes = height;
	unsigned char* y_src_line0 = src_addr;
	unsigned char* uv_src_line0 = src_addr + width*height;
...
void nv_rotage270(unsigned int width, unsigned int height,
	unsigned char* src_addr, unsigned char* dst_addr)
{
#define ROTAGE_270_NV21 1
#define ROTAGE_270_YU12 0

	unsigned int i = 0;
	unsigned int j = 0;
	unsigned int k = 0;

	unsigned int format = ROTAGE_270_NV21;
	unsigned int row_bytes = width;
	unsigned int col_bytes = height;
	...
	} else if (format == ROTAGE_270_YU12) {
		for (i = 0; i < height/2; i++) {
			for (j = 0; j < width/2; j++) {
				*(uv_dst_line0) = *(uv_src_line0++);
				uv_dst_line0 += height/2;
