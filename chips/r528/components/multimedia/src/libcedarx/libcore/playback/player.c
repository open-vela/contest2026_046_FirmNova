                           ...
                           int                  nStreamIndex)
{
	...
	int64_t vsubmit_time1 = GetNowUs();
        ret = VideoDecCompSubmitStreamData(p->pVideoDecComp, &dataInfo, nStreamIndex);
			
	int64_t vsubmit_time2 = GetNowUs();
	if(vsubmit_time2 -vsubmit_time1 > 15000)
	{
		logw("wht>>>demux,video Submit timeout:%lldms\n",(vsubmit_time2 - vsubmit_time1)/1000);
	}
        return ret;
#endif
	...
	int64_t asubmit_time1 = GetNowUs();
        ret = AudioDecCompSubmitStreamData(p->pAudioDecComp, &dataInfo, nStreamIndex);
	
	int64_t asubmit_time2 = GetNowUs();
	if(asubmit_time2 -asubmit_time1 > 15000)
	{
		logw("wht>>>demux,audio Submit timeout:%lldms\n",(asubmit_time2 - asubmit_time1)/1000);
	}
        ...
        case PLAYER_VIDEO_RENDER_NOTIFY_PICTURE_PTS:
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    loge("%d video frames not sync, force sync. vpts(%.3f) jztime(%.3f)",
                            p->nUnsyncVideoFrameCnt, nVideoPts/1000000.0, nCurTime/1000000.0);
                }
            }
...
static int PlayerInitialVideo(PlayerContext* p)
{
    ...
    p->sVideoConfig.nDeInterlaceHoldingFrameBufferNum = 2;
    p->sVideoConfig.nRotateHoldingFrameBufferNum      = 0;
    p->sVideoConfig.nDecodeSmoothFrameBufferNum       = 3;
#endif
    ...
    logv("nVbvBufferSize=%d, nAvgByteRate=%d",
     ...
     //vbv_size = GetConfigParamterInt("vbv_buffer_size", 3);
     vbv_size = 3;
     if(vbv_size != 0)
     {
    ...
    logv("wht>>>>debug, display_hold = %d, smoothframe = %d, nVbvBufferSize = %d", p->sVideoConfig.nDisplayHoldingFrameBufferNum, p->sVideoConfig.nDecodeSmoothFrameBufferNum, p->sVideoConfig.nVbvBufferSize);
