...
VideoDecComp* VideoDecCompCreate(void)
{
    ...
    err = pthread_create(&p->sDecodeThread, &attr, VideoDecodeThread, p);
    if(err != 0)
    {
    ...
    pthread_setname_np(p->sDecodeThread, "VideoDecode");
                                 ...
                                 int                  nStreamIndex)
{
    ...
    {
	logv("wht>>>>>>>>>debug, nSemCnt = %d", nSemCnt);
        if(nSemCnt == 0)
            sem_post(&p->streamDataSem);
    }
#endif
...
static void doDecode(AwMessage *msg, void *arg)
{
    ...
    int64_t video_dec2 = GetNowUs();
    if((video_dec2 - video_dec1) > 15000)
    {
	logv("wht>>>video_dec, dec timeout:%lldms\n",((video_dec2 - video_dec1)/1000));
    }

    logv("DecodeVideoStream ret = %d, p->bCrashFlag(%d)", ret, p->bCrashFlag);
    ...
    if(ret == VDECODE_RESULT_NO_BITSTREAM)
    {
        ...
        {
	logv("wht>>>>>>>>debug, no video bit stream ,wait !!!, usleep 100 ms");
            CdxSemTimedWait(&p->streamDataSem, 20);    //* wait for stream data.//20  5000
            BaseCompContinue(&p->base);
            return;
        }
    }
    ...
    {
	logv("wht>>>>>>>>debug, no frame buffer ,wait !!!, usleep 20 ms");
        CdxSemTimedWait(&p->frameBufferSem, 20);    //* wait for frame buffer. 20  2
        BaseCompContinue(&p->base);
        return;
    }
