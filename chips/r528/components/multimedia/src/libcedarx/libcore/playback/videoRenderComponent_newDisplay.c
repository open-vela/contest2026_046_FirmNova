...
VideoRenderComp* VideoRenderCompCreate(void)
{
    ...
    param.sched_priority = 240;//46
    pthread_attr_init(&attr);
    pthread_attr_setschedparam(&attr, &param);
    pthread_attr_setstacksize(&attr, 32768);
    err = pthread_create(&p->sRenderThread, &attr, VideoRenderThread, p);
    if(err != 0)
    {
    ...
    pthread_setname_np(p->sRenderThread, "VideoRender");
    return p;
}
...
int VideoRenderCompSetDeinterlace(VideoRenderComp* p, Deinterlace* pDi)
{
    ...
    logw("video render component setting deinterlace: %p",pDi);
...
int VideoRenderCompSetWindow(VideoRenderComp* p, LayerCtrl* lc)
{
    ...
    sem_init(&p->window_replySem, 0, 0);

    AwMessage msg = {
        .messageId = MESSAGE_ID_SET_WINDOW,
        .replySem = &p->window_replySem,
        .opaque = lc,
        .execute = handleSetWindow,
    };

    logw("video render component setting window: %p", lc);
    ...
    if(CdxSemTimedWait(&p->window_replySem, -1) < 0)
    {
        loge("video render component wait for setting window finish failed.");
        sem_destroy(&p->window_replySem);
        return -1;
    }
...
{
    sem_init(&p->hide_replySem, 0, 0);

    AwMessage msg = {
        .messageId = MESSAGE_ID_SET_VIDEO_HIDE,
        .replySem = &p->hide_replySem,
...
int VideoRenderSetHoldLastPicture(VideoRenderComp* p, int bHold)
{
    ...
    logw("video render component setting hold last picture(bHold=%d).", bHold);
...
static void* VideoRenderThread(void* arg)
{
    VideoRenderComp *p = arg;
    AwMessage msg;
    logv("wht>>>>>debug, go to get message");
    while (AwMessageQueueGetMessage(p->mq, &msg) == 0)
    {
    logv("wht>>>>>debug, get message ok!!");
        if (msg.execute != NULL)
            msg.execute(&msg, p);
        else
...
static int requestPicture(VideoRenderComp *p)
{
	...
	logv("dec comp request pic fail!!,wait 5 ms");
    int64_t req_t3 = GetNowUs();
        if (AwMessageQueueWaitMessage(p->mq, 5) > 0)//5
            return -1;

    int64_t req_t4 = GetNowUs();
    if((req_t4 - req_t3) > 15000)
    {
	loge("wht>>>Render,RequestPicture AwMessageQueueWaitMessage timeout:%lldms\n",(req_t4 - req_t3)/1000);
    }
...
static int notifyFirstPts(VideoRenderComp *p)
{
     ...
     */
    logw("wht>>>warning, notifyFirstPts");
...
static int showProgressivePicture(VideoRenderComp *p)
{
    ...
    logw("wht>>>debug, the first picture is showed unsychronized, p->nSetToDecoderBufferNum= %d,  p->nGpuBufferNum= %d", p->nSetToDecoderBufferNum , p->nGpuBufferNum);
    ...
    int64_t show_t1 = GetNowUs();
    int ret = ProcessVideoSync(p, p->pPicture);

    int64_t show_t2 = GetNowUs();

    if((show_t2 - show_t1) > 15000)
    {
	logv("wht>>>Render, video sync timediff:%lldms\n", (show_t2 - show_t1)/1000);
    }
...
static void doRender(AwMessage *msg, void *arg)
{
    ...
    int64_t render_t1 = GetNowUs();
    if (checkFlags(p) != 0)
        return;
    int64_t render_t2 = GetNowUs();
    if((render_t2 - render_t1) > 15000)
    {
	logv("wht>>>Render,check flags timeout:%lldms\n",(render_t2 - render_t1)/1000);
    }
    ...
    if (p->pPicture == NULL)
    {
        ...
        {
    int64_t render_t3 = GetNowUs();
if((render_t3 - render_t2) > 15000)
    {
	logv("wht>>>Render,request pic timeout:%lldms\n",(render_t3 - render_t2)/1000);
    }
    ...
    int64_t render_t4 = GetNowUs();
    if((render_t4 - render_t2) > 15000)
    {
	logv("wht>>>Render,request pic timeout:%lldms\n",(render_t4 - render_t2)/1000);
    }
    ...
    int64_t render_t6 = GetNowUs();

    if((render_t6 - render_t5) > 15000)
    {
	logv("wht>>>Render, p->bDeinterlaceFlag = %d, show picture timediff:%lldms\n",p->bDeinterlaceFlag, (render_t6 - render_t5)/1000);
    }
...
static inline int ProcessVideoSync(VideoRenderComp* p, VideoPicture* pPicture)
{
	...
	int64_t wait_t0 = GetNowUs();
    nWaitTime = p->callback(p->pUserData, PLAYER_VIDEO_RENDER_NOTIFY_PICTURE_PTS,
            (void*)&pPicture->nPts);

	int64_t wait_t1 = GetNowUs();
		if((wait_t1 - wait_t0) > 15000)
		{
			loge("wht>>>Render, PLAYER_VIDEO_RENDER_NOTIFY_PICTURE_PTS timeout:%lldms\n",(wait_t1 - wait_t0)/1000);
		}
	...
	int64_t wait_t5 = GetNowUs();
	int msg_ret = AwMessageQueueWaitMessage(p->mq, nWaitTime);

	int64_t wait_t6 = GetNowUs();
	if((wait_t6 - wait_t5) > 15000)
	{
	logv("wht>>>Render,wait zzzzzzz nWaitTime timeout:%lldms\n",(wait_t6 - wait_t5)/1000);
	}
...
#if 0
	int64_t wait_t7 = GetNowUs();
		if((wait_t7 - wait_t5) > 15000)
		{
			logw("wht>>>Render,wait zzzzzzz nWaitTime timeout:%lldms\n",(wait_t7 - wait_t5)/1000);
		}
#endif

    }
#if !defined(CONF_PRODUCT_STB)
    else if(nWaitTime < -50)//50
    {
...
static int QueueBufferToShow(VideoRenderComp* p, VideoPicture* pPicture)
{
    if(p->pLayerCtrl != NULL)
    {
        ...
        if(frameEndTime - p->mStartCountDispFrameTime<1000000){
            ...
            logv("lc->mDispFrameCount = %lld,frameStartTime = %lld ,frameEndTime = %lld \n",p->mDispFrameCount, p->mStartCountDispFrameTime,frameEndTime);
