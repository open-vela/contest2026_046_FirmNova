...
AudioRenderComp* AudioRenderCompCreate(void)
{
    ...
    param.sched_priority = 45;//16  17
    pthread_attr_init(&attr);
    pthread_attr_setschedparam(&attr, &param);
    pthread_attr_setstacksize(&attr, 32768);//32768

    err = pthread_create(&p->sRenderThread, &attr, AudioRenderThread, p);
    if(err != 0)
    {
    ...
    pthread_setname_np(p->sRenderThread, "AudioRender");
    return p;
}
...
static inline int requestPcmData(AudioRenderComp *p)
{
    ...
    int64_t audio_req_pcm_time1 = GetNowUs();
        ret = AudioDecCompRequestPcmData(p->pDecComp, &pPcmData,
                        &nPcmDataLen, &threadCtx->nPts, &p->cfg);

    int64_t audio_req_pcm_time2 = GetNowUs();

    if(audio_req_pcm_time2 - audio_req_pcm_time1 > 15000)
    {
	logw("wht>>>Render, AudioDecCompRequestPcmData timeout:%lldms, ret =%d\n",(audio_req_pcm_time2 - audio_req_pcm_time1)/1000, ret);
    }
	    ...
	    logv("wht>>>>>>debug, no pcm data, wait some time.");
...
static inline int checkSampleRate(AudioRenderComp *p)
{
    ...
    if(nNewChannelNum == 1){
      logv("the channel num is 1, maybe something is wrong!");
      return -1;
    }
...
static void doRender(AwMessage *msg, void *arg)
{
    ...
    //*******************************************
    int64_t req_pcm_time1 = GetNowUs();
    if (requestPcmData(p) != 0)
        return;

    int64_t req_pcm_time2 = GetNowUs();

    if(req_pcm_time2 -req_pcm_time1 > 15000)
    {
	logw("wht>>>Render, timeout:%lldms\n",(req_pcm_time2 - req_pcm_time1)/1000);
    }
