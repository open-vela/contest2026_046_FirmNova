...
#define rdLockPlaylist(hlsParser) \
    do { \
        if (pthread_mutex_lock(&hlsParser->rwlockPlaylist)) \
            logd("pthread_rwlock_timedrdlock error: %s", strerror(errno)); \
    } while (0)
...
#define rdUnlockPlaylist(hlsParser) \
    do { \
        pthread_mutex_unlock(&hlsParser->rwlockPlaylist); \
    } while (0)
#endif
...
static int DownloadParseM3u8(CdxHlsParser *hlsParser)
{
...
#else
    mbedtls_md5_context m;
    mbedtls_md5_init(&m);
    mbedtls_md5_starts(&m);
    mbedtls_md5_update(&m, hlsParser->m3u8Buf, m3u8ReadSize);
#endif
    ...
    if(hlsParser->isMasterParser == -1)
    {
	    ...
	    mbedtls_md5_finish(&m, hlsParser->u.media.mMD5);
...
static cdx_int32 HlsParserClose(CdxParserT *parser)
{
    ...
    pthread_mutex_destroy(&hlsParser->rwlockPlaylist);
    free(hlsParser);
    return 0;
}
...
static CdxParserT *HlsParserOpen(CdxStreamT *stream, cdx_uint32 flags)
{
    ...
    ret = pthread_mutex_init(&hlsParser->rwlockPlaylist, NULL);
    CDX_FORCE_CHECK(ret == 0);
