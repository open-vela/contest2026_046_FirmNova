...
_GetES:
    while(!mTSParser->currentES)
    {
        ...
        do
        {
            ...
            if(ret < 0)
            {
                CDX_LOGE("TSResync fail, try again");
                //goto _exit;
                goto _GetES;
            }
            else if(ret == 0)
...
//#define PROBE_PACKET_NUM (42*1024) should not too big
#define PROBE_PACKET_NUM (4*1024)
...
CdxParserT *TSParserOpen(CdxStreamT *cdxStream, cdx_uint32 flags)
{
    ...
    cdx_uint32 bufSize = PROBE_PACKET_NUM * 128;//
