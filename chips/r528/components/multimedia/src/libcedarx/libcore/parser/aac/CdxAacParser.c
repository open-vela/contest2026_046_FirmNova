...
int AACFindSyncWord1(unsigned char *pBuf, int nBytes)
{
    int i;
...
int AACFindSyncWord_LATM1(unsigned char *pBuf, int nBytes)
{
    int i;
...
int GetNextFrame1(AacParserImplS *impl )
{
    int nBytes;
    int retVal=-1;
    int nRead = 0;
    ...
    if(impl->ulformat == AAC_FF_ADTS)
    {
        nBytes = AACFindSyncWord1(impl->readPtr,impl->bytesLeft);
        if(nBytes<0)
        {
            return -1;
        }
    ...
    {
        nBytes = AACFindSyncWord_LATM1(impl->readPtr,impl->bytesLeft);
        if(nBytes<0)
        {
            return -1;
        }
...
//                }
            ...
            nBytes = AACFindSyncWord1(readPtr,bytesLeft);
            if(nBytes<0)
            {
                CDX_LOGE("AACFindSyncWord1 error");
                return -1;
            }
            nSyncLen  += nBytes;
            readPtr   += nBytes;
    ...
    else//latm
    {
        while(retVal<0)
        {
            ...
            nBytes = AACFindSyncWord_LATM1(readPtr,bytesLeft);
            if(nBytes<0)
            {
...
//But cannot delete it now.
                ...
                nBytes = AACFindSyncWord1(impl->readBuf,READLEN - nBytes);//maybe 4*1024;
                if(nBytes<0)
                {
                    CDX_LOGE("AACFindSyncWord1 error");
                    goto AAC_error;
...
//But cannot delete it now.
                ...
                nBytes = AACFindSyncWord_LATM1(impl->readBuf,READLEN - nBytes);
...
static cdx_int32 __AacParserControl(CdxParserT *parser, cdx_int32 cmd, void *param)
{
    ...
    case CDX_PSR_CMD_STREAM_SEEK:
        ...
        if(ret<0)
        {
            CDX_LOGE("Controll GetNextFrame1 error");
            return CDX_FAILURE;
        }
        impl->eofReached = 0;
