                     ...
                     cdx_uint32 pts_num, cdx_uint32 pts_den)
{
    ...
    if(av_reduce(&s->time_base.num, &s->time_base.den, pts_num, pts_den, OGG_INT_MAX))
    {
...
static AVStream *av_new_stream(CdxOggParser *ogg, int id)
{
    ...
    st = cdx_av_mallocz(sizeof(AVStream));
    if (!st)
        return NULL;
...
static int ogg_new_stream(CdxOggParser *ogg, uint32_t serial)
{
    ...
    CDX_LOGD("serial=0x%lx, idx=%d", serial, idx);
    return idx;
}
...
/**´ËÊ±Ó¦ÊÇÁíÒ»ÎïÀíÁ÷£¬ÒòÎªÍ¬Ò»ÎïÀíÁ÷µÄ¸÷Âß¼­Á÷µÄbosÊÇÏàÁÚµÄ
...
static int ogg_replace_stream(CdxOggParser *ogg, uint32_t serial, int nsegs)
{
    ...
    if (j >= ogg->nstreams)
    {
        CDX_LOGW("replace_stream fail");//getmediainfoÖ»ÔÚ¿ªÊ¼Ê±×ö¹ý£¬ËùÒÔ¶Ô´ËnewStreamÊÇÎÞÐ§µÄ
        ogg->mErrno = PSR_EOS;
        return AVERROR_EOF;
    }
...
_nextPage:
    ...
    ret = CdxStreamRead(bc, os->buf + os->bufpos, size);//ÕæµÄ½«page data¶ÁÈë
    if (ret < (int)size)
    {
        CDX_LOGE("CdxStreamRead fail, ret(%d)", ret);
        if(ret < 0)
