                                 ...
                                 const char *title)
{
    ...
    if(!pCpt){
        pCpt= cdx_av_mallocz(sizeof(AVChapter));
        if(!pCpt)
            return NULL;
...
cdx_uint32 cdx_av_xiphlacing(unsigned char *s, cdx_uint32 v)
{
    cdx_uint32 n = 0;
...
static int vorbis_header(CdxOggParser *ogg, int idx)
{
    ...
    if (!os->private) {
        os->private = cdx_av_mallocz(sizeof(struct oggvorbis_private));
        if (!os->private)
        {
            return 0;
        }
    ...
    p->packet[pkt_type >> 1] = cdx_av_mallocz(os->psize);
