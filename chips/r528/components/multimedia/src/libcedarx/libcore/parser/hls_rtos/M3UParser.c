...
static void trim(AString *line)
{
    ...
    cdx_uint32 i = 0;
    while (i < line->mSize && isspace((int)line->mData[i]))
    {
        ++i;
    }
    cdx_uint32 j = line->mSize;
    while (j > i && isspace((int)line->mData[j - 1]))
    {
        --j;
    }
...
int createAtomPool(AtomPool *p)
{
    ...
    for (i = 0; i < MAX_ATOM_NUM_IN_POOL; i++)
    {
        p->patom[i] = (Atom *)malloc(sizeof(Atom));
        if (p->patom[i] == NULL)
        {
            goto err;
        }
    }
    pthread_mutex_init(&p->atomlock, NULL);
    return 0;
err:
    for (i = 0; i < MAX_ATOM_NUM_IN_POOL; i++)
    {
...
void destroyAtomPool(AtomPool *p)
{
    ...
    pthread_mutex_destroy(&p->atomlock);
}

Atom *getAtomFromPool(AtomPool *p)
{
    int i;
    Atom *tmp_atom = NULL;
    ...
    if (p == NULL)
    {
        tmp_atom = malloc(sizeof(Atom));
        return tmp_atom;
    }

    pthread_mutex_lock(&p->atomlock);
    for (i = 0; i < MAX_ATOM_NUM_IN_POOL; i++)
    {
        if (p->patom[i])
        {
    ...
    pthread_mutex_unlock(&p->atomlock);

    if (tmp_atom == NULL)  /* pool is empty now, malloc one */
    {
        tmp_atom = malloc(sizeof(Atom));
    }
...
void putAtomToPool(AtomPool *p, Atom *patom)
{
    int i;
    Atom *tmp_atom = patom;
    ...
    if (tmp_atom != NULL) /* pool is full, free it */
    {
        free(tmp_atom);
    }
}
...
static status_t setInt32(AMessage *meta, const char *key, cdx_int32 int32Value)
{
    Atom *matom;
#if HLS_IOT_USE_ATOM_POOL
    matom = getAtomFromPool(meta->pool);
#else
    matom = malloc(sizeof(Atom));
#endif
    if (matom == NULL)
        return ERROR_maxNumAtom_too_little;
    memset(matom, 0, sizeof(Atom));

    matom->u.int32Value = int32Value;
    matom->mType = kTypeInt32;
    matom->mName = key;

    if (meta->mAtom == NULL)
        meta->mAtom = matom;
    else {
        Atom *tmp_atom = meta->mAtom;
        while (tmp_atom->next != NULL)
            tmp_atom = tmp_atom->next;
        tmp_atom->next = matom;
    }
...
static status_t setInt64(AMessage *meta, const char *key, cdx_int64 int64Value)
{
    Atom *matom;
#if HLS_IOT_USE_ATOM_POOL
    matom = getAtomFromPool(meta->pool);
    ...
    matom->u.int64Value = int64Value;
    matom->mType = kTypeInt64;
    matom->mName = key;
...
static status_t setString(AMessage *meta, const char *key, AString *stringValue)
{
    Atom *matom;
#if HLS_IOT_USE_ATOM_POOL
    matom = getAtomFromPool(meta->pool);
    ...
    matom->u.stringValue = stringValue;
    matom->mType = kTypeString;
    matom->mName = key;
...
{
    Atom *matom = meta->mAtom;
    while (matom) {
        if (!strcmp(matom->mName, name) && (matom->mType == kTypeInt32)) {
            *value = matom->u.int32Value;
            return 1;
        }
        matom = matom->next;
    }
...
int findInt64(const AMessage *meta, const char *name, cdx_int64 *value)
{
    Atom *matom = meta->mAtom;
    while (matom) {
        if (!strcmp(matom->mName, name) && (matom->mType == kTypeInt64)) {
            *value = matom->u.int64Value;
            return 1;
        }
        matom = matom->next;
    }
...
int findString(const AMessage *meta, const char *name, AString **value)
{
    Atom *matom = meta->mAtom;
    while (matom) {
        if (!strcmp(matom->mName, name) && (matom->mType == kTypeString)) {
            *value = matom->u.stringValue;
            return 1;
        }
        matom = matom->next;
    }
...
static void destroyPlaylistItems(PlaylistItem *p)
{
    ...
    while (p)
    {
        e = p->next;
#if HLS_MALLOC_ATOM_DYNAMIC
        Atom *matom = p->itemMeta.mAtom;
        while (matom) {
            Atom *tmp_atom;
            if (matom->mType == kTypeString) {
                destroyAString(matom->u.stringValue);
            ...
            tmp_atom = matom->next;
#if HLS_IOT_USE_ATOM_POOL
            putAtomToPool(p->itemMeta.pool, matom);
#else
            free(matom);
#endif
            matom = tmp_atom;
...
#if HLS_MALLOC_ATOM_DYNAMIC
        Atom *matom = curPlayList->mMeta.mAtom;
        while (matom) {
            Atom *tmp_atom;
            if (matom->mType == kTypeString) {
                destroyAString(matom->u.stringValue);
            ...
            tmp_atom = matom->next;
#if HLS_IOT_USE_ATOM_POOL
            putAtomToPool(curPlayList->mMeta.pool, matom);
#else
            free(matom);
#endif
            matom = tmp_atom;
                                ...
                                uint64_t *length, uint64_t *offset)
{
    ...
    if (atPos == NULL)
    {
        ...
        *length = strtoull(colonPos + 1, &pEnd, 10);
        if (!strcmp(pEnd, colonPos + 1) || (*pEnd != '\0' && !isspace((int)*pEnd)))
            return ERROR_MALFORMED;
        *offset = curOffset;
    }
    else
    {
        char *pEnd;
        *length = strtoull(colonPos + 1, &pEnd, 10);
        if (!strcmp(pEnd, colonPos + 1) || (*pEnd != '@' && *pEnd != '\0' && !isspace((int)*pEnd)))
            return ERROR_MALFORMED;
...
/* iot:c library does not has api memmem, so we achieve it here */
static void *memmem(const char *a, unsigned int alen, const char *b, unsigned int blen)
{
    int i, j;
...
{
    cdx_uint32 offset = 0;
    while (offset < size && isspace((int)data[offset])) //isspace中包含‘\n’\r,所以已经排除了空行的情况
    {
        offset++;
    }
    if(offset + 7 >= size)
...
#endif
{
    *P = NULL;
    ...
    while (offset < size)
    {
        ...
        while(isspace((int)data[offsetData-1]))
        {
            --offsetData;
