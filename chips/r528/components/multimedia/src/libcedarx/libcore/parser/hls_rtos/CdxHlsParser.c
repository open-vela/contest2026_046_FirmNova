...
#define OPTION_HLS_IOT_OPTIMIZE_REFRESH_WAITTIME 1
...
#define HLS_SUPPORT_SWITCH_BANDWIDTH_DYNAMICALLY   0
...
#define TMALLPLAYER_DEFAULT_PRIORITY 28
#define TMALLPLAYER_NORMAL_STACKSIZE 65536 //32768

static int CallbackProcess(void* pUserData, int eMessageId, void* param);

#define rdLockPlaylist(hlsParser) \
    do { \
        if (pthread_mutex_lock(&hlsParser->lockPlaylist)) \
...
#define rdUnlockPlaylist(hlsParser) \
    do { \
        pthread_mutex_unlock(&hlsParser->lockPlaylist); \
    } while (0)

#define wrUnlockPlaylist(hlsParser) rdUnlockPlaylist(hlsParser)

static uint32_t const iv[4] = {0x67452301, 0xefcdab89, 0x98badcfe, 0x10325476};
                               ...
                               CdxStreamT **stream, ContorlTask *streamTasks, int64_t timeout)
{
    ...
    int count = -1;
    source->protectSize = 0;
    source->bufferSize = 16 * 1024;
_reTry:
    count++;
    ...
    if(playlist->u.mediaPlaylistPrivate.hasEncrypte)
    {
        ...
        if(cipherReference != hlsParser->u.media.cipherReference)
        {
            ...
            if(!strcasecmp(cipherMethod->mData, "AES-128"))
            {
                ...
                streamContorlTask.cmd = STREAM_CMD_NO_PROBE_DATA;
                streamContorlTask.param = NULL;
                ...
                ret = CdxStreamOpen_Retry(&dataSource, &hlsParser->statusLock,
                                          &hlsParser->forceStop, &stream, &streamContorlTask,
                ...
                if(findString(&cipherReference->itemMeta, "cipher-iv", &cipherIv))
                {
                    ...
                    for (i = 0; i < 16; ++i)
                    {
                        char c_1 = (char)tolower((int)(cipherIv->mData[2 + 2 * i]));
                        char c_2 = (char)tolower((int)(cipherIv->mData[3 + 2 * i]));
                        if (!isxdigit((int)c_1) || !isxdigit((int)c_2))
                        {
                            CDX_LOGE("malformed cipher IV (%s)", cipherIv->mData);
                            ret = -1;
                            goto out;
                        }
                        cdx_uint8 nb1 = isdigit((int)c_1) ? c_1 - '0' : c_1 - 'a' + 10;
                        cdx_uint8 nb2 = isdigit((int)c_2) ? c_2 - '0' : c_2 - 'a' + 10;
...
static int DownloadParseM3u8(CdxHlsParser *hlsParser)
{
    ...
    mbedtls_md5_update(&md5_ctx, hlsParser->m3u8Buf, m3u8ReadSize);
    ...
    if(hlsParser->isMasterParser == -1)
    {
        ...
        if ((err = M3u8Parse(hlsParser->m3u8Buf, m3u8ReadSize, &playlist, hlsParser->m3u8Url, &hlsParser->atomPool)) !=
             OK)
#else
        ...
        {
            mbedtls_md5_finish(&md5_ctx, hlsParser->u.media.mMD5);
    ...
    {
        uint8_t key[16];
        mbedtls_md5_finish(&md5_ctx, key);
...
static int64_t RefreshPlaylistWaitTime(CdxHlsParser *hlsParser)
{
...
#if OPTION_HLS_IOT_OPTIMIZE_REFRESH_WAITTIME
    ...
    int32_t curSeqNumber = hlsParser->u.media.curSeqNum;
    int64_t remain_duration = 0;

    if ((curSeqNumber < firstSeqNumberInPlaylist - 1) || (curSeqNumber >= lastSeqNumberInPlaylist)) {
        remain_duration = 0;
    } else {
        int i;
        for (i = curSeqNumber + 1;i <= lastSeqNumberInPlaylist;i++) {
            PlaylistItem *item = findItemBySeqNum(playlist, i);
            if (item == NULL)
                break;
            remain_duration += item->u.mediaItemAttribute.durationUs;
        }
        /* the items in playlist can still play remain_duration microsecond at least.
         * and 2s will be enough to finish refreshing playlist.
         */
        remain_duration -= 2000000;
    }
#endif
    ...
    rdUnlockPlaylist(hlsParser);
    wait_time = hlsParser->u.media.mLastPlaylistFetchTimeUs + minAgeUsPlaylist - CdxGetNowUs();

#if OPTION_HLS_IOT_OPTIMIZE_REFRESH_WAITTIME
    if (remain_duration > wait_time)
...
static int RefreshPlaylist(CdxHlsParser *hlsParser)
{
    ...
    if (hlsParser->parser_init == 0) {
        if (hlsParser->cdxStream) {
            if (CdxStreamControl(hlsParser->cdxStream, STREAM_CMD_GET_PARSER_INIT, NULL) == 0) {
                pthread_mutex_unlock(&hlsParser->statusLock);
                return -1;
            } else {
                hlsParser->parser_init = 1;
            }
        }
    }
...
*/
    ...
    ContorlTask streamContorlTask, streamContorlTask1, streamContorlTask2;
    ...
    streamContorlTask1.next = &streamContorlTask2;
    streamContorlTask2.cmd = STREAM_CMD_NO_PROBE_DATA;
    streamContorlTask2.param = NULL;
    streamContorlTask2.next = NULL;
...
static void *PeriodicTaskThread(void *arg)
{
    ...
    while (1)
    {
        ...
        if (RefreshPlaylist(hlsParser) < 0)
        {
            ...
            if (forceStop)
                break;
#endif
            /* wait 20ms to refresh again */
            struct timespec time;
            clock_gettime(CLOCK_REALTIME, &time);
            waitTime = 20 * 1000L * 1000L + time.tv_nsec;
            time.tv_sec += waitTime / (1000L * 1000L * 1000L);
            time.tv_nsec = waitTime % (1000L * 1000L * 1000L);
            if (sem_timedwait(&hlsParser->PeriodicTask.semStop, &time) == 0)
                break;
    ...
    _info("PeriodicTaskThread stack left: %d\n", OS_ThreadGetStackMinFreeSize(NULL));
#endif
            ...
            if(getGroupItemUriFromPlaylist(playlist, selectTrace, &mURI))
            {
                if(strcmp(mURI->mData, item->mURI->mData))
                {
                    ...
                    else
                    {
                        ...
                        hlsParser->cdxDataSource.protectSize = 0;
                        hlsParser->cdxDataSource.bufferSize = 0;
...
static int CallbackProcess(void* pUserData, int eMessageId, void* param)
{
    ...
    switch(eMessageId)
    {
        ...
        {
            //int flag = *(int *)param;
            //logd("EVT: STREAM_EVT_DETAIL_INFO, RPT: STREAM_EVT_DETAIL_INFO, cb:%08lx, flag:%d", hlsParser->callback, flag);
            break;
        }
        default:
...
_NextSegment:
                ...
                if ((hlsParser->childStream[0]) && (hlsParser->bufferFreeValid)) {
                    CdxStreamControl(hlsParser->childStream[0], STREAM_CMD_FREE_BUFFER, NULL);
                }
#endif
...
static cdx_int32 HlsParserSeekTo(CdxParserT *parser, cdx_int64  timeUs)
{
    ...
    if(hlsParser->isMasterParser)
    {
        ...
        for(i = 0; i < 3; i++)
        {
            if(hlsParser->child[i])
            {
                ret = CdxParserSeekTo(hlsParser->child[i], timeUs, AW_SEEK_PREVIOUS_SYNC);
                if(ret < 0)
                {
...
static cdx_int32 HlsParserClose(CdxParserT *parser)
{
...
#if (HLS_IOT_USE_ATOM_POOL && HLS_MALLOC_ATOM_DYNAMIC)
    destroyAtomPool(&hlsParser->atomPool);
#endif
    ...
    pthread_mutex_destroy(&hlsParser->lockPlaylist);
    free(hlsParser);
    return 0;
}
...
static int HlsParserInit(CdxParserT *parser)
{
    ...
    if(!hlsParser->isMasterParser)
    {
        ...
        if (!playlist->u.mediaPlaylistPrivate.mIsComplete)
        {
            ...
            if (pthread_create(&hlsParser->PeriodicTask.tid, &attr,
                        PeriodicTaskThread, (void *)hlsParser) == 0){
                pthread_setname_np(hlsParser->PeriodicTask.tid, "Hls");
...
#if !HLS_SUPPORT_SWITCH_BANDWIDTH_DYNAMICALLY
        hlsParser->callback(hlsParser->pUserData,
                             PARSER_NOTIFY_AUDIO_URL_CHANGE, (void *)item->mURI->mData);
#endif
...
static const struct CdxParserOpsS hlsParserOps =
{
...
static CdxParserT *HlsParserOpen(CdxStreamT *stream, cdx_uint32 flags)
{
...
#if (HLS_IOT_USE_ATOM_POOL && HLS_MALLOC_ATOM_DYNAMIC)
    if (createAtomPool(&hlsParser->atomPool)) {
        CDX_LOGE("create atom pool fail\n");
        goto open_error;
    }
#endif
    ...
    ret = pthread_mutex_init(&hlsParser->lockPlaylist, NULL);
    CDX_FORCE_CHECK(ret == 0);
...
const CdxParserCreatorT hlsParserCtor =
{
