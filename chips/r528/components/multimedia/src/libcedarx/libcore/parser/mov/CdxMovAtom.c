...
const char * const cdx_id3v1_genre_str[ID3v1_GENRE_MAX + 1] = {
      [0] = "Blues",
      [1] = "Classic Rock",
      [2] = "Country",
      [3] = "Dance",
...
static void movParseGnre(unsigned char *buffer, CDX_U8 *str, int size)
{
    ...
    genre = buffer[offset];
    CDX_LOGD("--- genre %s", cdx_id3v1_genre_str[genre-1]);
    snprintf((char*)str, size, "%s", cdx_id3v1_genre_str[genre-1]);
}
...
static int _MovParseFtyp(MOVContext *s,unsigned char *buf,MOV_atom_t a)
{
    ...
    if(strcmp(compatible, "M4A mp42isom") == 0 || strcmp(compatible, "mp42isom") ==0 || strcmp(compatible, "isomiso2") == 0)
    {
	CDX_LOGE("not support this format!!!");
        return -1;
    }
