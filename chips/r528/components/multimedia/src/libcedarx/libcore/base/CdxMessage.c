...
void DeliverTimerInit(void)
{
    ...
    ret = pthread_create(&gDeliverTimerHdr->timeoutProcPid, &attr,
                        DeliverTimerProcess, gDeliverTimerHdr);
    pthread_setname_np(gDeliverTimerHdr->timeoutProcPid, "DeliverTimerProcess");
...
CdxDeliverT *CdxDeliverCreate(AwPoolT *pool)
{
    ...
    pthread_attr_setschedparam(&attr, &param);
    pthread_attr_setstacksize(&attr, 32768);
    ret = pthread_create(&impl->pid, &attr, DeliverProcess, impl);
    pthread_setname_np(impl->pid, "DeliverProcess");
