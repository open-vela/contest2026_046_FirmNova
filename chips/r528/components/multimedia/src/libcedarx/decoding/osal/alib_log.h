...
#define __OS_LINUX 1
    ...
    #ifndef __ANDROID__ 
        #define alib_loge(fmt, arg...) _info("(%s),line(%d) : "fmt"\r\n", LOG_TAG, __LINE__, ##arg)
        #define alib_logd(fmt, arg...) if(ALIB_DEBUG > 0) _info("(%s),line(%d) : "fmt"\r\n", LOG_TAG, __LINE__, ##arg)
        #define alib_logw(fmt, arg...) if(ALIB_DEBUG > 1) _info("(%s),line(%d) : "fmt"\r\n", LOG_TAG, __LINE__, ##arg)
        #define alib_logv(fmt, arg...) if(ALIB_DEBUG > 2) _info("(%s),line(%d) : "fmt"\r\n", LOG_TAG, __LINE__, ##arg)
