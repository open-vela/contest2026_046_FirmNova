...
typedef struct __OutputPcmBuf
{
    ...
    char  *pAOutPcmEndPtr; //last byte of pcm data of address in buffer
...
typedef struct AudioDecoderContextStruct
{
    ...
    int (*CheckAudioDecVersion)(AudioDecoderLib* pDecoder);
#if ( __ANDROID__ != 1 )
    AudioMix AMX;
#endif
}AudioDecoderContext;
...
{
    int cmpResult = 0;
    char* audioVersion = "adbac52f9a9871357a1c4912610edad40f87d473";

    if(pAudioDecoder->CheckAudioDecVersion(audioVersion))
    {
        alib_loge("audioVersion check fail");
        return -1;
    }
...
static int AdCedarInitFunction(AudioDecoderContext *pAudioDecoder)
{
	...
	pAudioDecoder->CheckAudioDecVersion             = CheckAudioVersion               ;
...
int PlybkRequestPcmBuffer(AudioDecoder* pDecoder, unsigned char **pOutReadPtr, int *psize)
{
    ...
    //alib_logd("*psize: %d, nAOutBufferValidSize: %d, nBitStreamUnderFlow: %d", (int)*psize, (int)pAudioDecoder->ADCedarCtxA.nAOutBufferValidSize, (int)pAudioDecoder->pBsInFor->nBitStreamUnderFlow);
...
int64_t PlybkRequestPcmPts(AudioDecoder* pDecoder)
{
    ...
    if(pAudioDecoder->pBsInFor->out_channels && pAudioDecoder->pBsInFor->out_samplerate)
    {
        pts = (pAudioDecoder->pBsInFor->nDecodeMode == CDX_DECODER_MODE_RAWMUSIC ? (int64_t)pAudioDecoder->pBsInFor->NowPlayTime * 1000 - (int64_t)pAudioDecoder->ADCedarCtxA.nAOutBufferValidSize * (500000) / (pAudioDecoder->pBsInFor->out_channels * pAudioDecoder->pBsInFor->out_samplerate)
              : (pAudioDecoder->ADCedarCtxA.nAOutBufferValidSize ? pAudioDecoder->ADCedarCtxA.frmFifo.inFrames[pAudioDecoder->ADCedarCtxA.frmFifo.rdIdx].pts
    ...
    if( ret == 0 )
    {
        if((!pAudioDecoder->raw_data.nNeedDirect)&&(pAudioDecoder->pBsInFor->modeflag != NORMAL_MIXALBE_16BIT_PCM))//init only raw
        {
            ...
            switch(pAudioDecoder->nCodeType)
            {
                ...
                case AUDIO_CODEC_FORMAT_DRA://maybe 7.1, default pcm
                default:
