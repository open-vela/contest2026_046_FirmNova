...
static void PrintDemoUsage(void)
{
    ...
    while(i < num)
    {
        _info("%s %-32s %s", ArgumentMapping[i].Short, ArgumentMapping[i].Name,
...
int main(int argc, char** argv)
{
    ...
    _info("***************************************************************\n");
    _info("*******test begin time[%llds] *******\n", time_begin);
    _info("***************************************************************\n");
    _info("\n");
    for(m=0; m<cycle_num; m++)
    {
        time_start = GetNowUs()/1000/1000;
        _info("*************[%u] cycle demo start time[%llds]**************\n", m,time_start);
        ...
        {
            _info("MemAdapterGetOpsS failed\n");
            goto out;
        }
    ...
    out:
        ...
        if(encode_param.compare_flag)
        {
            ...
            _info("the %u cycle test, freq:%d\n", m, encode_param.frequency);

            if(encode_param.compare_result)
            {
                _info("encoder: ve_freq[%dMHz], the compare result is ok\n", encode_param.frequency);
                if(log_file)
                {
            ...
            {
                _info("encoder: ve_freq[%dMHz], the compare result is fail\n", encode_param.frequency);
                time_end = GetNowUs()/1000/1000;
                _info("*******demo end time[%llds] cur_cycle_time[%llds]*******\n",     \
        ...
        time_end = GetNowUs()/1000/1000;
        _info("*******[%u]cycle demo end time[%llds] cur_cycle_time[%llds]*******\n",
    ...
    _info("\n");
    _info("***************************************************************\n");
    _info("*******test end time[%llds] test_total_time[%llds]*******\n",
