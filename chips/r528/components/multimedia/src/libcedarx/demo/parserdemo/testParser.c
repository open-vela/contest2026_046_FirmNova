...
int DemoParser(const char* url,const char* filePath)
{
    ...
    if(strlen(url) > 1023)
    {
        _info("url is too long!\n");
        free(buf);
        return -1;
    }
    if(strlen(filePath) > (1023 - sizeof("video.h264")))
    {
        _info("file path is too long!\n");
        free(buf);
        return -1;
    }
    strcpy(tmp,url);
    ...
    _info("url :%s",source.uri);
    ret = CdxParserPrepare(&source, 0, NULL, &forceExit,
                        &parser, &stream, NULL, NULL);
    if(ret < 0 || !parser)
    {
        _info("we can not get the right parser, maybe we not support this file\n");
        free(buf);
        return -1;
    }
    ...
    while(0 == CdxParserPrefetch(parser, &packet))
    {
        ...
        {
            _info("error: lenth(%d) > 1024*1024", packet.length);
            free(buf);
            return -1;
        }
...
{
    _info("********** useage *********\n");
    _info("param 1: the file path,like this :\"file:///mnt/sdcard/A098.mp4\" \n");
    _info("param 2: the dump file path,parser will store file in this path,"
