...
static __disp_pixel_fmt_t convert_pixel_format(DISP_INPUT_FORMAT fbFmt)
{
	...
	default:
		...
		_info("%s: unkown input format, fbFmt=%d\n", __func__, fbFmt);
		break;
	}
...
static INT32S disp_video_addr_set(INT32U fbAddr, INT32U img_width, INT32U img_height, DISP_INPUT_FORMAT fbFmt)
{
	...
	if (0 == disp_video_start_flag) {
		_info("%s: video is stop, cannot show video now\n", __func__);
		return -2;
	}
	...
	ret = ioctl(disp_fd, DISP_CMD_VIDEO_SET_FB, (void*)ioctlParam);
	if(ret != 0){
		_info("%s:ioctl disp disp off failed!\r\n", __func__);
		ret = -3;
		goto errHdl;
	}
...
{
	_info("m:%d brs:%d pipe:%d alen:%d alval:%d srcx:%d srcy:%d srcw:%d srch:%d scnx:%d scny:%d scnw:%d scnh:%d a0:%x a1:%x a2:%x fbformat:%d fbw:%d fbh:%d\n",
...
INT32S disp_rotate_set(INT32U angle)
{
	...
	if(ret != 0){
		_info("%s: set rotate failed, angle=%u!\r\n", __func__, angle);
		ret = -2;
		goto errHdl;
	}
...
INT32S disp_open_backlight(void)
{
	...
	if (disp_fd <= 0) {
		_info("%s: disp device has not open yet.\n", __func__);
		return -1;
	}
