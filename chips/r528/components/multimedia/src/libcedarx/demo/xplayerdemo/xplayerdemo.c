...
static const Command commands[] =
{
    ...
    {"P",           COMMAND_PAUSE,              "pause the playback."},
    {"S",            COMMAND_STOP,               "stop the playback."},
    {"s",         COMMAND_SEEKTO,
        "seek to specific position to play, position is in unit of second, ex, seek to: 100."},
    {"i", COMMAND_SHOW_MEDIAINFO,  "show media information of the media file."},
    {"G",   COMMAND_SHOW_DURATION,   "show duration of the media file."},
    {"g",   COMMAND_SHOW_POSITION,   "show current play position,in unit of second."},
    {NULL, 0, NULL}
};
#if 0
static const Command commands[] =
...
static void showHelp(void)
{
    ...
    _info("\n");
    _info("******************************************************************************\n");
    _info("* This is a simple media player, when it is started, you can input commands to tell\n");
    _info("* what you want it to do.\n");
    _info("* Usage: \n");
    _info("*   # ./demoPlayer\n");
    _info("*   # set url: http://www.allwinner.com/ald/al3/testvideo1.mp4\n");
    _info("*   # show media info\n");
    _info("*   # play\n");
    _info("*   # pause\n");
    _info("*   # stop\n");
    _info("*\n");
    _info("* Command and it param is seperated by a colon, param is optional, as below:\n");
    _info("*     Command[: Param]\n");
    _info("*\n");
    _info("* here are the commands supported:\n");
        ...
        case COMMAND_SEEKTO:
            if(strParam != NULL)
            {
                *pParam = (int)strtol(strParam, (char**)NULL, 10);  //* seek time in unit of second.
                ...
                {
                    _info("seek time is not valid.\n");
                    nCommandId = -1;
                }
            }
            else
            {
                _info("no seek time is specified.\n");
                nCommandId = -1;
            }
            break;
        ...
        case COMMAND_SWITCH_AUDIO:
            if(strParam != NULL)
            {
                *pParam = (int)strtol(strParam, (char**)NULL, 10);
                if(errno == EINVAL || errno == ERANGE)
                {
                    _info("audio stream index is not valid.\n");
                    nCommandId = -1;
                }
            }
            else
            {
                _info("no audio stream index is specified.\n");
                nCommandId = -1;
            }
            break;
...
int CallbackForAwPlayer(void* pUserData, int msg, int ext1, void* param)
{
    ...
    switch(msg)
    {
        case AWPLAYER_MEDIA_INFO:
        {
            switch(ext1)
            {
                case AW_MEDIA_INFO_NOT_SEEKABLE:
                {
                    ...
                    _info("info: media source is unseekable.\n");
                    break;
                }
                case AW_MEDIA_INFO_RENDERING_START:
                {
                    _info("info: start to show pictures.\n");
                    break;
                }
            }
            break;
        ...
        case AWPLAYER_MEDIA_ERROR:
        {
            ...
            _info("error: open media source fail.\n");
        ...
        case AWPLAYER_MEDIA_SET_VIDEO_SIZE:
        {
            ...
            _info("++++ video width: %d, height: %d \n", w, h);
            break;
        }

        default:
        {
            //_info("warning: unknown callback from AwPlayer.\n");
            break;
        }
    }
...
int main(int argc, char** argv)
{
    ...
    {
        _info("can not create AwPlayer, quit.\n");
        exit(-1);
    }
    ...
    {
        _info("initCheck of the player fail, quit.\n");
        ...
        if(readCommand(strCommandLine, sizeof(strCommandLine)) == 0)
        {
            ...
            switch(nCommandId)
            {
                ...
                case COMMAND_SET_SOURCE :   //* set url of media file.
                {
                    ...
                    {
                        _info("error:\n");
                        _info("    AwPlayer::setDataSource() return fail.\n");
                        break;
                    }
                     _info("setDataSource end\n");
                    ...
                    {
                        _info("error:\n");
                        _info("    AwPlayer::prepareAsync() return fail.\n");
                ...
                case COMMAND_PLAY:   //* start playback.
                {
                    ...
                    {
                        _info("error:\n");
                        _info("    AwPlayer::start() return fail.\n");
                        break;
                    }
                ...
                case COMMAND_PAUSE:   //* pause the playback.
                {
                    ...
                    {
                        _info("error:\n");
                        _info("    AwPlayer::pause() return fail.\n");
                        break;
                    }
                    demoPlayer.mPreStatus = demoPlayer.mStatus;
                ...
                case COMMAND_STOP:   //* stop the playback.
                {
                    ...
                    {
                        _info("error:\n");
                        _info("    AwPlayer::reset() return fail.\n");
                        break;
                    }
                    demoPlayer.mPreStatus = demoPlayer.mStatus;
                    demoPlayer.mStatus    = STATUS_STOPPED;
                ...
                case COMMAND_SEEKTO:   //* seek to posion, in unit of second.
                {
                    ...
                    {
                        _info("getDuration fail, unable to seek!\n");
                        break;
                    }

                    if(nSeekTimeMs > nDuration)
                    {
                        _info("seek time out of range, media duration = %d seconds.\n",
                                nDuration/1000);
                        break;
                    }
                    ...
                    {
                        _info("media source is unseekable.\n");
                        break;
                    }
                ...
                case COMMAND_SHOW_DURATION:   //* show media duration, in unit of second.
                {
                    int nDuration = 0;
                    if(XPlayerGetDuration(demoPlayer.mAwPlayer, &nDuration) == 0)
                        _info("media duration = %d seconds.\n", nDuration/1000);
                    else
                        _info("fail to get media duration.\n");
                    break;
                }
                ...
                case COMMAND_SHOW_POSITION:   //* show current play position, in unit of second.
                {
                    ...
                    if(XPlayerGetCurrentPosition(demoPlayer.mAwPlayer, &nPosition) == 0)
                        _info("current position = %d seconds.\n", nPosition/1000);
                    else
                        _info("fail to get pisition.\n");
                    break;
                }
                ...
                case COMMAND_SWITCH_AUDIO:   //* switch autio track.
                {
                    ...
                    _info("switch audio to the %dth track.\n", nAudioStreamIndex);
                    //* TODO
                    break;
                }
