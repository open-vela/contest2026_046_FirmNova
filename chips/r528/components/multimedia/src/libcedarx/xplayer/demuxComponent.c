    ...
    switch(eMessageId)
    {
        ...
        case PARSER_NOTIFY_VIDEO_STREAM_CHANGE:
        {
            ...
            if(demux->cacheThreadId != 0)
            {
                ...
                CdxSemTimedWait(&demux->semStreamChange, -1);
            }
            else
#endif
            {
...
#if 0
	case PARSER_NOTIFY_AUDIO_URL_CHANGE:
	{
		int nMsg = DEMUX_AUDIO_URL_CHANGE;
		demux->callback(demux->pUserData, nMsg, param);
		break;
	}
#endif
...
DemuxComp* DemuxCompCreate(void)
{
...
#else
    d->nStartPlayCacheSize     = 128*1024;
    d->nCacheBufferSize        = 20*1024*1024;
    d->nCacheBufferSizeLive    = 20*1024*1024;
    d->nStartPlayCacheTime     = 0; // we will set it later
    d->nMaxStartPlayChacheSize = 150*1024*1024;
    d->nMaxCacheBufferSize     = 200*1024*1024;
    ...
    pthread_attr_setstacksize(&attr, 65536);//32768
    if(pthread_create(&d->threadId, &attr, DemuxThread, (void*)d) != 0)
    {
    ...
    pthread_setname_np(d->threadId, "Demux");
#if CONFIG_ONLINE_SUPPORT
...
void DemuxCompDestroy(DemuxComp* d)
{
    ...
    AwMessageQueuePostMessage(demux->mq, &msg);
    CdxSemTimedWait(&demux->semQuit, -1);
...
void DemuxCompClear(DemuxComp* d)  //* clear the data source, like just created.
{
    ...
    AwMessageQueuePostMessage(demux->mq, &msg);
    CdxSemTimedWait(&demux->semClear, -1);
            ...
            const char* pUrl, const CdxKeyedVectorT* pHeaders)
{
    ...
    AwMessageQueuePostMessage(demux->mq, &msg);
    CdxSemTimedWait(&demux->semSetDataSource, -1);
...
int DemuxCompPrepareAsync(DemuxComp* d)
{
    ...
    //loge("wht>>debug, go to post demux mq, demux->mq = 0x%08x", demux->mq);
    AwMessageQueuePostMessage(demux->mq, &msg);
    return 0;
}
...
int DemuxCompCancelPrepare(DemuxComp* d)
{
    ...
    AwMessageQueuePostMessage(demux->mq, &msg);
    CdxSemTimedWait(&demux->semCancelPrepare, -1);
    return 0;
}
#ifndef ONLY_ENABLE_AUDIO
...
&& demux->bBufferring != 1
    ...
    AwMessageQueuePostMessage(demux->mq, &msg);
    CdxSemTimedWait(&demux->semStart, -1);
...
int DemuxCompStop(DemuxComp* d)    //* close the data source, must call prepare again to restart.
{
    ...
    AwMessageQueuePostMessage(demux->mq, &msg);
    CdxSemTimedWait(&demux->semStop, -1);
...
int DemuxCompCancelSeek(DemuxComp* d)  //* should not call back DEMUX_SEEK_FINISH message.
{
    ...
    msg.params[1] = 0; //* no reply.
    AwMessageQueuePostMessage(demux->mq, &msg);
    CdxSemTimedWait(&demux->semCancelSeek, -1);
    return 0;
}
...
#else
	pBuffering->mRebufferingWatermarkHighKB = 20*1024;
        pBuffering->mRebufferingWatermarkLowKB = 512;
#endif
...
#if !CONFIG_ONLY_DISABLE_AUDIO
        nBitrate += PlayerGetAudioBitrate(demux->pPlayer);
#endif
...
static void* DemuxThread(void* arg)
{
    ...
    CdxPacketT          packet;
    enum EMEDIATYPE     ePlayerMediaType = MEDIA_TYPE_VIDEO;
...
process_message:
	...
	logv("wht>>debug, get msg.messageId = %d", msg.messageId);
...
//
            ...
            if(demux->bFileStream == 0)
                demux->nStartPlayCacheTime = 6*1000;
...
_endTask:
            ...
            if(demux->bCancelPrepare)
            {
	    loge("[debug] go to demux prepare");
        ...
        else if(msg.messageId == DEMUX_COMMAND_STOP)
        {
            ...
            if(demux->cacheThreadId != 0)
            {
                ...
                AwMessageQueuePostMessage(demux->mqCache, &newMsg);
                CdxSemTimedWait(&demux->semCache, -1);
        ...
        else if(msg.messageId == DEMUX_COMMAND_RESET_URL)
        {
		    ...
		    logw("DEMUX_NOTIFY_BUFFER_END");
                ...
                while(demux->pParser == NULL)
                {
                    ...
                    //ret = CdxSemTimedWait(&demux->semCache, 50);
                ...
                CdxSemTimedWait(&demux->semCache, -1); // wait DEMUX_COMMAND_RESET_URL in cache thread
            }
...
#endif
#ifndef ONLY_ENABLE_AUDIO
                        if(node->eMediaType == CDX_MEDIA_SUBTITLE)
                        {
...
#endif
                if(packet.type == CDX_MEDIA_METADATA)
                {
                    ePlayerMediaType = MEDIA_TYPE_METADATA;
                    nStreamIndex     = packet.streamIndex;
                }
                
                if(packet.type == CDX_MEDIA_UNKNOWN)
                {
                    loge("media type from parser not valid, should not run here, abort().");
                    abort();
                }
		...
		int64_t psr_read1 = GetNowUs();
                ret = CdxParserRead(demux->pParser, &packet);
		int64_t psr_read2 = GetNowUs();
		if(psr_read2 -psr_read1 > 15000)
		{
			logv("wht>>>Demux,Parser read timeout:%lldms\n",(psr_read2 - psr_read1)/1000);
		}
                if(ret == 0)
                {
                    if(
...
#endif
			//int64_t submit_time1 = GetNowUs();
                        PlayerSubmitStreamData(demux->pPlayer, &streamDataInfo,
                                            ePlayerMediaType, nStreamIndex);
			//int64_t submit_time2 = GetNowUs();
			//if(submit_time2 -submit_time1 > 15000)
			//{
			//	logv("wht>>>demux,Submit timeout:%lldms\n",(submit_time2 - submit_time1)/1000);
			//}
