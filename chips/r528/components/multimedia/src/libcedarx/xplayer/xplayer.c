...
unsigned int XPlayer_Playerback_Timeout_ms = 60000;
unsigned int XPlayer_Prepare_Timeout_ms = 15000;
unsigned int XPlayer_Tcpread_Timeout_ms = 0xffffffff;
//* player status.
...
XPlayer* XPlayerCreate(void)
{
    ...
    param.sched_priority = 240;//  48
    pthread_attr_init(&attr);
    pthread_attr_setschedparam(&attr, &param);
    pthread_attr_setstacksize(&attr, 32768);

    if(pthread_create(&mPriData->mThreadId, &attr, XPlayerThread, mPriData) == 0)
        mPriData->mThreadCreated = 1;
    else
        mPriData->mThreadCreated = 0;
    pthread_setname_np(mPriData->mThreadId, "XPlayer");
#ifndef ONLY_ENABLE_AUDIO
...
void XPlayerDestroy(XPlayer* p)
{
    ...
    if(mPriData->mThreadCreated)
    {
        ...
        CdxSemTimedWait(&mPriData->mSemQuit, -1);
                            ...
                            const CdxKeyedVectorT* pHeaders)
{
    ...
    AwMessageQueuePostMessage(mPriData->mMessageQueue, &msg);
    CdxSemTimedWait(&mPriData->mSemSetDataSource, -1);
...
int XPlayerSetVideoSurfaceTexture(XPlayer* p, const LayerCtrl* surfaceTexture)
{
    ...
    AwMessageQueuePostMessage(mPriData->mMessageQueue, &msg);
    CdxSemTimedWait(&mPriData->mSemSetSurface, -1);

    return mPriData->mSetSurfaceReply;
}
#endif

#if !CONFIG_ONLY_DISABLE_AUDIO
void XPlayerSetAudioSink(XPlayer* p, const SoundCtrl* audioSink)
{
    ...
    AwMessageQueuePostMessage(mPriData->mMessageQueue, &msg);
    CdxSemTimedWait(&mPriData->mSemSetAudioSink, -1);
...
int XPlayerSetPlaybackSettings(XPlayer* p,const XAudioPlaybackRate *rate)
{
    ...
    AwMessageQueuePostMessage(mPriData->mMessageQueue, &msg);
    CdxSemTimedWait(&mPriData->mSemSetPlayBackSettings, -1);
...
void XPlayerSetSubCtrl(XPlayer* p, const SubCtrl* subctrl)
{
    ...
    AwMessageQueuePostMessage(mPriData->mMessageQueue, &msg);
    CdxSemTimedWait(&mPriData->mSemSetSubCtrl, -1);
...
void XPlayerSetDeinterlace(XPlayer* p, const Deinterlace* di)
{
    ...
    AwMessageQueuePostMessage(mPriData->mMessageQueue, &msg);
    CdxSemTimedWait(&mPriData->mSemSetDeinterlace, -1);
...
int XPlayerPrepareAsync(XPlayer* p)
{
    ...
    msg.params[2] = 0;
    AwMessageQueuePostMessage(mPriData->mMessageQueue, &msg);
    CdxSemTimedWait(&mPriData->mSemPrepare, -1);
...
int XPlayerPrepare(XPlayer* p)
{
    ...
    if(mPriData->mPrepareReply == 0)
    {
        ...
        CdxSemTimedWait(&mPriData->mSemPrepareFinish, -1);
...
int XPlayerStart(XPlayer* p)
{
    ...
    AwMessageQueuePostMessage(mPriData->mMessageQueue, &msg);
    CdxSemTimedWait(&mPriData->mSemStart, -1);
...
int XPlayerStop(XPlayer* p)
{
    ...
    AwMessageQueuePostMessage(mPriData->mMessageQueue, &msg);
    CdxSemTimedWait(&mPriData->mSemStop, -1);
...
int XPlayerPause(XPlayer* p)
{
    ...
    else if(mPriData->mLivemode == 2)
    {
        ...
        logd("get current position in pause, nowUs = %" PRId64 ", mPauseTimeStamp = %" PRId64 "",
#else
        logd("get current position in pause, nowUs = %" PRId32 ", mPauseTimeStamp = %" PRId64 "",
#endif
    ...
    AwMessageQueuePostMessage(mPriData->mMessageQueue, &msg);
    CdxSemTimedWait(&mPriData->mSemPause, -1);
...
int XPlayerSeekTo(XPlayer* p, int nSeekTimeMs, SeekModeType nSeekModeType)
{
    ...
    if(mPriData->mLivemode == 1 || mPriData->mLivemode == 2)
    {
        ...
        //* params[3] = mSeekSync.
        AwMessageQueuePostMessage(mPriData->mMessageQueue, &msg);
        CdxSemTimedWait(&mPriData->mSemSeek, -1);
    }
    else
#endif
#endif
...
int XPlayerReset(XPlayer* p)
{
    ...
    AwMessageQueuePostMessage(mPriData->mMessageQueue, &msg);
    CdxSemTimedWait(&mPriData->mSemReset, -1);
...
int XPlayerSetSpeed(XPlayer* p, int nSpeed)
{
    ...
    AwMessageQueuePostMessage(mPriData->mMessageQueue, &msg);
    CdxSemTimedWait(&mPriData->mSemSetSpeed, -1);
...
#ifdef CONFIG_SYSTEM_TIME64
                logd("livemode2, nowUs = %" PRId64 ", mPauseTimeStamp = %" PRId64 "",
#else
                logd("livemode2, nowUs = %" PRId32 ", mPauseTimeStamp = %" PRId64 "",
#endif
                        tv.tv_sec * 1000000 + tv.tv_usec, mPriData->mPauseTimeStamp);
            }
            else
...
static void* XPlayerThread(void* arg)
{
	...
	pReplySem   = (sem_t*)msg.params[0];
        ...
        if(msg.messageId == XPLAYER_COMMAND_SET_SOURCE)
        {
            ...
            logd("xplayer:process message XPLAYER_COMMAND_SET_SOURCE.\n");
            //* check status.
            if(mPriData->mStatus != XPLAYER_STATUS_IDLE &&
                    mPriData->mStatus != XPLAYER_STATUS_INITIALIZED)
            {
        ...
        else if(msg.messageId == XPLAYER_COMMAND_SET_SURFACE)
        {
            ...
            logd("xplayer:process message XPLAYER_COMMAND_SET_SURFACE.\n");
        ...
        else if(msg.messageId == XPLAYER_COMMAND_SET_AUDIOSINK)
        {
            ...
            logd("xplayer:process message XPLAYER_COMMAND_SET_AUDIOSINK.\n");
        ...
        else if(msg.messageId == XPLAYER_COMMAND_SET_SUBCTRL)
        {
            ...
            logd("xplayer:process message XPLAYER_COMMAND_SET_SUBCTRL.\n");
        ...
        else if(msg.messageId == XPLAYER_COMMAND_SET_DI)
        {
            ...
            logd("xplayer:process message XPLAYER_COMMAND_SET_DI.\n");
        ...
        else if(msg.messageId == XPLAYER_COMMAND_PREPARE)
        {
            logd("process message XPLAYER_COMMAND_PREPARE. mPriData->mStatus: %d",
                  mPriData->mStatus);
            logd("xplayer:process message XPLAYER_COMMAND_PREPARE.\n");
            if(mPriData->mStatus == XPLAYER_STATUS_PREPARED)
            {
        ...
        else if(msg.messageId == XPLAYER_COMMAND_SETSPEED)
        {
            ...
            logd("xplayer:process message XPLAYER_COMMAND_SETSPEED.\n");
            if(mPriData->mStatus != XPLAYER_STATUS_PREPARED &&
               mPriData->mStatus != XPLAYER_STATUS_STARTED )
            {
        ...
        else if(msg.messageId == XPLAYER_COMMAND_START)
        {
            ...
            logd("xplayer:process message XPLAYER_COMMAND_START.\n");
            if(mPriData->mStatus != XPLAYER_STATUS_PREPARED &&
               mPriData->mStatus != XPLAYER_STATUS_STARTED  &&
               mPriData->mStatus != XPLAYER_STATUS_PAUSED   &&
               mPriData->mStatus != XPLAYER_STATUS_COMPLETE)
        ...
        else if(msg.messageId == XPLAYER_COMMAND_STOP)
        {
            ...
            logd("xplayer:process message XPLAYER_COMMAND_STOP.\n");
            if(mPriData->mStatus != XPLAYER_STATUS_PREPARED &&
               mPriData->mStatus != XPLAYER_STATUS_STARTED  &&
               mPriData->mStatus != XPLAYER_STATUS_PAUSED   &&
               mPriData->mStatus != XPLAYER_STATUS_COMPLETE &&
        ...
        else if(msg.messageId == XPLAYER_COMMAND_PAUSE)
        {
            ...
            logd("xplayer:process message XPLAYER_COMMAND_PAUSE.\n");
            if(mPriData->mStatus != XPLAYER_STATUS_STARTED  &&
               mPriData->mStatus != XPLAYER_STATUS_PAUSED   &&
               mPriData->mStatus != XPLAYER_STATUS_COMPLETE)
            {
        ...
        else if(msg.messageId == XPLAYER_COMMAND_RESET)
        {
            ...
            logd("xplayer:process message XPLAYER_COMMAND_RESET.\n");
             //* the prepare callback may happen at this moment.
            //* so the mStatus may be changed to PREPARED asynchronizely.
            if(mPriData->mStatus == XPLAYER_STATUS_PREPARING)
            {
        ...
        else if(msg.messageId == XPLAYER_COMMAND_SEEK)
        {
            ...
            logd("xplayer:process message XPLAYER_COMMAND_SEEK.\n");
            if(mPriData->mStatus != XPLAYER_STATUS_PREPARED &&
               mPriData->mStatus != XPLAYER_STATUS_STARTED  &&
               mPriData->mStatus != XPLAYER_STATUS_PAUSED   &&
               mPriData->mStatus != XPLAYER_STATUS_COMPLETE)
        ...
        else if(msg.messageId == XPLAYER_COMMAND_RESETURL)
        {
            ...
            logd("xplayer:process message XPLAYER_COMMAND_RESETURL.\n");
            if(mPriData->mStatus != XPLAYER_STATUS_PREPARED &&
               mPriData->mStatus != XPLAYER_STATUS_STARTED  &&
               mPriData->mStatus != XPLAYER_STATUS_PAUSED   &&
               mPriData->mStatus != XPLAYER_STATUS_COMPLETE)
    ...
    if(nStreamNum > 0 && pNewStreamInfo[0].eCodecFormat == SUBTITLE_CODEC_IDXSUB)
    {
        if(fdSub >= 0)
        {
            ...
            for(i=0; i<nStreamNum; i++)
                ...
                //pNewStreamInfo[i].fdSub = vfs_dup(fdSub);
        }
        else
        {
...
#ifndef ONLY_ENABLE_AUDIO
        int 	nBitrate;
        nBitrate = PlayerGetVideoBitrate(mPriData->mPlayer);
#if !CONFIG_ONLY_DISABLE_AUDIO
        nBitrate += PlayerGetAudioBitrate(mPriData->mPlayer);
#endif
        return nBitrate;
#else
#if !CONFIG_ONLY_DISABLE_AUDIO
...
static int callbackProcess(void *p, int messageId, void* param)
{
    ...
    switch(messageId)
    {
		...
		logv("debug demux prepare success, initialize the player.");
        ...
        case DEMUX_AUDIO_STREAM_CHANGE:
            ...
            updateAudioInfo(p);
            break;
	case DEMUX_AUDIO_URL_CHANGE:
        {
	    mPriData->mCallback(mPriData->pUser, AWPLAYER_MEDIA_CHANGE_URL, 0, param);
	    break;
	}
#ifndef ONLY_ENABLE_AUDIO
