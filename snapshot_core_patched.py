...
def expand_codex_event(raw_event: dict, fallback_ts: str) -> list[dict]:
    """Convert one Codex CLI session-log event to zero-or-more contest events.

    Codex JSONL schema (VS Code / CLI):
      - top-level type: session_meta / turn_context / response_item / event_msg
      - response_item payload.type: message / reasoning / function_call / function_call_output
      - message payload: role (user/assistant/developer), content list (input_text/output_text)
      - reasoning payload: content list (reasoning_text)
      - function_call payload: name, arguments (JSON string), call_id
      - function_call_output payload: call_id, output (string)
    """
    out: list[dict] = []
    top_type = raw_event.get("type")
    ts = raw_event.get("timestamp") or fallback_ts
    ...
    if top_type in ("session_meta", "turn_context", "event_msg"):
        return out
    ...
    if ptype == "message":
        role = (payload.get("role") or "system").lower()
        # Map developer role to system for consistency
        if role == "developer":
            role = "system"
        ...
        for block in content:
            ...
            if bt in ("input_text", "output_text"):
                text = block.get("text", "")
                if text:
                    out.append({"ts": ts, "role": role, "text": text})
        return out
    ...
    if ptype == "reasoning":
        ...
        for block in content:
            ...
            if block.get("type") == "reasoning_text":
                text = block.get("text", "")
                if text:
                    out.append({"ts": ts, "role": "assistant", "thinking": text})
        return out

    if ptype == "function_call":
        name = payload.get("name", "unknown")
        call_id = payload.get("call_id", f"call_{len(out)}")
...
def process_claude_stdin(stdin_data: dict, tool: str, team_id: str) -> int:
    ...
    expand_fn = expand_codex_event if tool == "codex" else expand_claude_event
    contest_events = []
    for raw in new_raw:
        contest_events.extend(expand_fn(raw, fallback_ts=iso_now()))
...
def auto_export_to_repo(repo_root: Path, staging_member_dir: Path, staging_jsonl: Path,
    ...
    try:
        ...
        # Skip copy when staging and destination are the same file
        if staging_jsonl.resolve() != dest_jsonl.resolve():
