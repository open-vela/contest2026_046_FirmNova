...
#endif
#ifndef AGENT_SECRET_VOLC_API_KEY
#define AGENT_SECRET_VOLC_API_KEY ""
#endif
#ifndef AGENT_SECRET_VOLC_APP_ID
#define AGENT_SECRET_VOLC_APP_ID ""
#endif
#ifndef AGENT_SECRET_VOLC_TOKEN
#define AGENT_SECRET_VOLC_TOKEN ""
#endif
...
/* ── Data directories ────────────────────────────────────── */
/* Default path; runtime overridden via ai_agent_profile_t.data_dir */
#ifndef CONFIG_EXAMPLES_AI_AGENT_VELA_DATA_DIR
#define CONFIG_EXAMPLES_AI_AGENT_VELA_DATA_DIR "/data/agent"
#endif

#define AGENT_DATA_DIR_DEFAULT CONFIG_EXAMPLES_AI_AGENT_VELA_DATA_DIR
/* AGENT_DATA_DIR is now a runtime value; keep macro for static fallback only */
#define AGENT_DATA_DIR AGENT_DATA_DIR_DEFAULT
...
/* ── Node client identity — runtime override via ai_agent_profile_t ── */
...
/* Static fallback macros; ai_agent_lib.c uses profile->node_id at runtime */
#define AGENT_NODE_ID_DEFAULT          CONFIG_EXAMPLES_AI_AGENT_VELA_NODE_ID
#define AGENT_NODE_DISPLAY_NAME_DEFAULT CONFIG_EXAMPLES_AI_AGENT_VELA_NODE_DISPLAY_NAME
#define AGENT_NODE_ID          AGENT_NODE_ID_DEFAULT
#define AGENT_NODE_DISPLAY_NAME AGENT_NODE_DISPLAY_NAME_DEFAULT
...
#define AGENT_VOICE_CAPTURE_SAMPLE_RATE 24000
...
/* WebSocket TTS output sample rate (hardware pipeline rate).
 * Volcano TTS WS big-model voices output PCM at 24000Hz.
 * The sunxi audio driver does not support 24000Hz natively (period_size
 * constraint), so we use 48000Hz for the player and resample from
 * the actual 24000Hz server output via linear interpolation (2:1 ratio). */
#define AGENT_TTS_WS_SAMPLE_RATE 48000

/* Actual sample rate returned by the TTS WS server (24000Hz).
 * The resampler in audio_playback.c converts this to AGENT_TTS_WS_SAMPLE_RATE. */
#define AGENT_TTS_WS_ACTUAL_RATE 24000
...
#define AGENT_SHELL_SECURITY_ALLOWLIST 0
#define AGENT_SHELL_SECURITY_FULL 1
#define AGENT_SHELL_SECURITY_DENY 2

/* Support both legacy (packages/ai_agent) and new (services/ai_agent) symbols */
#if defined(CONFIG_RIVOTEK_AI_AGENT_SHELL_ALLOWLIST) || \
    defined(CONFIG_EXAMPLES_AI_AGENT_VELA_SHELL_ALLOWLIST)
#define AGENT_SHELL_SECURITY AGENT_SHELL_SECURITY_ALLOWLIST
#elif defined(CONFIG_RIVOTEK_AI_AGENT_SHELL_FULL) || \
      defined(CONFIG_EXAMPLES_AI_AGENT_VELA_SHELL_FULL)
#define AGENT_SHELL_SECURITY AGENT_SHELL_SECURITY_FULL
#elif defined(CONFIG_RIVOTEK_AI_AGENT_SHELL_DENY) || \
      defined(CONFIG_EXAMPLES_AI_AGENT_VELA_SHELL_DENY)
