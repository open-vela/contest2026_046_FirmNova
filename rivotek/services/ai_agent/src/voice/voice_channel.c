...
static size_t resample_24k_to_16k(const int16_t* input,
    size_t input_samples, int16_t* output, size_t output_cap,
    int16_t pending[3], int* pending_count)
{
    size_t output_samples = 0;

    for (size_t i = 0; i < input_samples; i++) {
        pending[(*pending_count)++] = input[i];
        if (*pending_count == 3) {
            if (output_samples + 2 > output_cap) {
                break;
            }

            output[output_samples++] = pending[0];
            output[output_samples++] = (int16_t)(
                ((int32_t)pending[1] + pending[2]) / 2);
            *pending_count = 0;
        }
    }
...
static void* recording_thread(void* arg)
{
    ...
    int16_t resampled[AGENT_ASR_CHUNK_SIZE / sizeof(int16_t)];
    int16_t resample_pending[3];
    int resample_pending_count = 0;
    unsigned char asr_chunk[AGENT_ASR_CHUNK_SIZE];
    size_t asr_chunk_len = 0;
    int need_fallback = 0;
    // int dump_fd = -1;
    ...
    while (1) {
        ...
        if (st != VOICE_RECORDING && st != VOICE_STOPPING) {
            break;
        }
        ...
        if (n <= 0) {
            ...
            if (n != -ECANCELED && st == VOICE_RECORDING) {
                syslog(LOG_WARNING,
        ...
        size_t resampled_samples = resample_24k_to_16k(
            (const int16_t*)chunk, (size_t)n / sizeof(int16_t),
            resampled, sizeof(resampled) / sizeof(resampled[0]),
            resample_pending, &resample_pending_count);
        size_t resampled_len = resampled_samples * sizeof(int16_t);

#ifdef CONFIG_AI_AGENT_AUDIO_PREPROCESS
    preproc_chunk(pp, (unsigned char*)resampled, resampled_len);
#endif

        size_t offset = 0;
        while (offset < resampled_len) {
            size_t copy_len = AGENT_ASR_CHUNK_SIZE - asr_chunk_len;
            size_t remaining = resampled_len - offset;

            if (copy_len > remaining) {
                copy_len = remaining;
            }

            memcpy(asr_chunk + asr_chunk_len,
                (unsigned char*)resampled + offset, copy_len);
            asr_chunk_len += copy_len;
            offset += copy_len;

            if (asr_chunk_len == AGENT_ASR_CHUNK_SIZE) {
                process_audio_chunk(&stream, asr_chunk, asr_chunk_len,
...
{
    struct tts_playback_context* context = user_data;
    ...
    if (pcm_data && pcm_len > 0 && context && !context->open_error) {
        if (!context->pb) {
            context->pb = audio_playback_open(
                AGENT_AUDIO_PLAYBACK_DEV,
                AGENT_TTS_WS_SAMPLE_RATE,
                AGENT_VOICE_CHANNELS,
                AGENT_VOICE_BITS,
                AGENT_TTS_WS_ACTUAL_RATE);
            if (!context->pb) {
                context->open_error = -EIO;
                return;
            }

            pthread_mutex_lock(&s_voice.lock);
            s_voice.tts_pb = context->pb;
            pthread_mutex_unlock(&s_voice.lock);
        }
        ...
        audio_playback_write(context->pb, pcm_data, pcm_len);
    }
...
int voice_channel_start(void)
{
    ...
    s_voice.cap = audio_capture_open(
        ...
        AGENT_VOICE_CAPTURE_SAMPLE_RATE,
        AGENT_VOICE_CHANNELS,
        AGENT_VOICE_BITS);
    ...
    if (audio_capture_start(s_voice.cap) < 0) {
        ...
        pthread_mutex_unlock(&s_voice.lock);
        audio_capture_stop(s_voice.cap);
...
int voice_channel_stop(void)
{
    ...
    /* Stop capture first so a pending FIFO read can exit. */
    audio_capture_stop(s_voice.cap);
...
int voice_channel_speak(const char* text)
{
     ...
     * so concurrent streams do not contend for the PCM device. */
    pthread_mutex_lock(&s_voice.lock);
    if (s_voice.tts_pb) {
    ...
    struct tts_playback_context playback = {0};
    ...
    /* Open playback lazily on the first PCM chunk.  Starting NxPlayer for a
     * server error or empty response leaves an empty FIFO stream that cannot
     * always be stopped by the lower-half driver. */
    int ret = voice_tts_speak_stream(clean,
        tts_stream_cb, &playback);
    ...
    if (playback.pb) {
        audio_playback_close(playback.pb);
    } else if (ret == 0) {
        ret = playback.open_error ? playback.open_error : -ENODATA;
        syslog(LOG_ERR, "[%s] TTS stream produced no audio: %d\n",
            TAG, ret);
    }
    free(clean);
