    ...
    size_t text_cap)
{
    ...
    int attempts = 0;
    int max_attempts = 1000; /* safety limit */
    int timeout_count = 0;

    while (attempts < max_attempts) {
        ret = recv_volc_response(&ctx, text_out, text_cap);
        if (ret < 0) {
            if (ret == -EIO && text_out[0] != '\0') {
                /* Socket timeout but we have text */
                break;
            }
            if (++timeout_count >= 3) {
                /* 3 consecutive timeouts — end of stream */
                break;
            }
            continue;
        }
    ...
    char* text_out, size_t text_cap)
{
    ...
    /* Receive responses until final (max 1000 to handle long recordings).
     * When all buffered responses are drained and socket times out (EIO),
     * treat as end-of-stream rather than hard error — the server may have
     * finished but not sent a final marker. */
    int attempts = 0;
    int timeout_count = 0;
    ...
    while (attempts < 1000) {
        ...
        if (ret < 0) {
            ...
            if (ret == -EIO && text_out[0] != '\0') {
                /* Socket timeout but we have text — good enough */
                break;
            }
            if (++timeout_count >= 3) {
                /* 3 consecutive timeouts — no more data */
                ret = 0; /* treat as clean finish */
                break;
            }
    ...
    if (attempts >= 1000) {
        syslog(LOG_WARNING,
            "[%s] stream: exhausted attempts without final\n",
            TAG);
    }

    if (text_out[0] == '\0') {
        syslog(LOG_WARNING,
            "[%s] stream: no text recognized\n",
            TAG);
        return -ENODATA;
    }
