...
#define TTS_MAX_CONSECUTIVE_TIMEOUTS 3
    ...
    while (1) {
        ...
        if (msg_type == VOLC_MSG_FRONTEND) {
            /* Frontend payload may contain JSON metadata (duration, sample_rate etc.) */
            size_t frontend_payload_len = flen - volc_hdr_len;
            if (frontend_payload_len > 0 && frontend_payload_len < 512) {
                char frontend_dump[512];
                size_t dump_len = frontend_payload_len < sizeof(frontend_dump) - 1
                    ? frontend_payload_len : sizeof(frontend_dump) - 1;
                memcpy(frontend_dump, buf + volc_hdr_len, dump_len);
                frontend_dump[dump_len] = '\0';
                syslog(LOG_INFO, "[%s] frontend: %s\n", TAG, frontend_dump);
            }
            if (chunks > 0) {
        ...
        if (msg_type == VOLC_MSG_AUDIO_RESP) {
            ...
            chunks++;
            total_pcm += pcm_len;
            syslog(LOG_DEBUG, "[%s] chunk %d: pcm_len=%zu total=%zu\n",
                TAG, chunks, pcm_len, total_pcm);
    ...
    syslog(LOG_INFO, "[%s] %d audio chunks delivered, total PCM: %zu bytes\n",
