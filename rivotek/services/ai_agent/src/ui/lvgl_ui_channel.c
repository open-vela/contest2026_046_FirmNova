...
/* ── Widget init semaphore ─────────────────────────────────── */

/* Forward declarations needed by lvgl_ui_thread */
static sem_t s_stop_sem;
static void lvgl_ui_do_cleanup(void);

/* All widget creation in one function — called from LVGL thread only */
static int lvgl_ui_create_widgets(void)
{
    int chat_h;

    s_state.screen = lv_obj_create(NULL);
    if (!s_state.screen) return -EIO;
    ...
    if (!s_state.chat_list) goto fail;
    ...
    if (!s_state.ptt_label) goto fail;
    ...
    if (!s_state.rec_indicator) goto fail;
    ...
    if (!s_state.close_btn) goto fail;
...
fail:
    if (s_state.screen) {
        ...
        s_state.screen = NULL;
        s_state.chat_list = s_state.ptt_btn = s_state.ptt_label = NULL;
        s_state.rec_indicator = s_state.close_btn = NULL;
    }
    return -EIO;
}

/* LVGL event loop thread — owns all LVGL calls including widget init */
static void *lvgl_ui_thread(void *arg)
{
    (void)arg;
    lv_nuttx_dsc_t dsc;
    lv_nuttx_result_t result;

    /* Each NuttX task needs its own lv_init() + lv_nuttx_init() to
     * populate the per-task TLS lv_global_t.  Without this, any LVGL
     * call crashes with LV_ASSERT because the global context is NULL. */
    lv_init();
    lv_nuttx_dsc_init(&dsc);
    dsc.fb_path = "/dev/lcd0";
    dsc.input_path = NULL;   /* no touch needed for agent UI */
    lv_nuttx_init(&dsc, &result);

    if (lvgl_ui_create_widgets() != 0) {
        syslog(LOG_ERR, "[%s] widget init failed\n", TAG);
    ...
    syslog(LOG_INFO, "[%s] event loop started\n", TAG);
    ...
    lv_deinit();
    sem_post(&s_stop_sem);
    syslog(LOG_INFO, "[%s] event loop exited\n", TAG);
    return NULL;
}
...
int lvgl_ui_channel_start(void)
{
    ...
    /* LVGL thread creates widgets + drives lv_timer_handler */
    if (agent_task_create(lvgl_ui_thread, "lvgl_ui",
                          AGENT_LVGL_UI_STACK, NULL,
                          AGENT_LVGL_UI_PRIO) != OK) {
        s_state.running = false;
        syslog(LOG_ERR, "[%s] failed to start LVGL thread\n", TAG);
        return -ENOMEM;
    }
