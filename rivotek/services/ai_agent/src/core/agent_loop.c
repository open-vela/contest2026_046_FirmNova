    ...
    const struct timespec* t1)
{
    ...
    return (uint32_t)sec_diff * 1000 + (uint32_t)nsec_diff / 1000000;
}
...
send_reply:
    ...
    if (final_text && !watchdog_fired && msg->content
        && complexity == LLM_COMPLEXITY_SIMPLE) {
