...
static int quit_cmd(void* handle, int argc, char** argv) { return -2; }
...
bt_instance_t* g_lightCtrl_ins = NULL;
...
static int set_adapter_cmd(void* handle, int argc, char** argv)
{
    ...
    int ret = execute_cmd_in_table(handle, g_set_cmd_tables, ARRAY_SIZE(g_set_cmd_tables), argc, argv);
    if (ret != CMD_OK)
        set_usage();
...
static int get_adapter_cmd(void* handle, int argc, char** argv)
{
    ...
    int ret = execute_cmd_in_table(handle, g_get_cmd_tables, ARRAY_SIZE(g_get_cmd_tables), argc, argv);
    if (ret != CMD_OK)
        get_usage();
...
static int pair_cmd(void* handle, int argc, char** argv)
{
    ...
    int ret = execute_cmd_in_table(handle, g_pair_cmd_tables, ARRAY_SIZE(g_pair_cmd_tables), argc, argv);
    if (ret != CMD_OK)
        pair_usage();
...
static void on_adapter_state_changed_cb(void* cookie, bt_adapter_state_t state)
{
    ...
    if (state == BT_ADAPTER_STATE_ON) {
        static char* arg_disconn[] = {"0"};
        gattc_disconnect_cmd(g_lightCtrl_ins, 1, arg_disconn);
        sleep(2);

        char name[64 + 1];

        bt_tool_init(g_lightCtrl_ins);
        /* get name */
        bt_adapter_get_name(g_lightCtrl_ins, name, 64);
        /* get io cap */
        bt_io_capability_t cap = bt_adapter_get_io_capability(g_lightCtrl_ins);
        /* get class */
        uint32_t class = bt_adapter_get_device_class(g_lightCtrl_ins);
        /* get scan mode */
        bt_scan_mode_t mode = bt_adapter_get_scan_mode(g_lightCtrl_ins);
        /* enable key derivation */
        bt_adapter_le_enable_key_derivation(g_lightCtrl_ins, true, true);
        bt_adapter_set_page_scan_parameters(g_lightCtrl_ins, BT_BR_SCAN_TYPE_INTERLACED, 0x400, 0x24);
        PRINT("Adapter Name: %s, Cap: %d, Class: 0x%08" PRIX32 ", Mode:%d", name, cap, class, mode);
        sleep(2);
        bt_adapter_set_scan_mode(g_lightCtrl_ins, 2, 1);
        PRINT("scan mode");
        sleep(2);
    } else if (state == BT_ADAPTER_STATE_TURNING_OFF) {
        /* code */
        bt_tool_uninit(g_lightCtrl_ins);
...
static void on_pair_request_cb(void* cookie, bt_address_t* addr)
{
    if (g_auto_accept_pair)
        bt_device_pair_request_reply(g_lightCtrl_ins, addr, true);
...
static void on_pair_display_cb(void* cookie, bt_address_t* addr, bt_transport_t transport, bt_pair_type_t type, uint32_t passkey)
{
    ...
    case PAIR_TYPE_PASSKEY_CONFIRMATION:
        ...
        ret = bt_device_set_pairing_confirmation(g_lightCtrl_ins, addr, transport, true);
...
{
    bt_device_connect_request_reply(g_lightCtrl_ins, addr, true);
...
int execute_cmd_in_table_offset(void* handle, bt_command_t* table, uint32_t table_size, int argc, char* argv[], uint8_t offset)
{
    int ret;
...
int execute_cmd_in_table(void* handle, bt_command_t* table, uint32_t table_size, int argc, char* argv[])
{
    return execute_cmd_in_table_offset(handle, table, table_size, argc, argv, 1);
}
...
static int bttool_ins_init(bttool_t* bttool)
{
    ...
    g_lightCtrl_ins = bluetooth_create_instance();
    if (g_lightCtrl_ins == NULL) {
        PRINT("create instance error\n");
        return -1;
    }

    adapter_callback = bt_adapter_register_callback(g_lightCtrl_ins, &g_adapter_cbs);
    if (bt_adapter_get_state(g_lightCtrl_ins) == BT_ADAPTER_STATE_ON)
        bt_tool_init(g_lightCtrl_ins);
...
{
    bt_tool_uninit(g_lightCtrl_ins);
    bt_adapter_unregister_callback(g_lightCtrl_ins, adapter_callback);
    bluetooth_delete_instance(g_lightCtrl_ins);
    g_lightCtrl_ins = NULL;
...
static void bttool_execute_command_cb(uv_async_queue_t* handle, void* buffer)
{
    ...
    if (_argc > 0) {
        if (bttool->async_api) {
            ...
            ret = execute_async_command(g_lightCtrl_ins, _argc, _argv);
#else
            ret = CMD_INVALID_OPT;
#endif
        } else
            ret = execute_command(g_lightCtrl_ins, _argc, _argv);
        if (ret != CMD_OK) {
            if (ret == -2) {
                if (bttool->async_api) {
#ifdef CONFIG_BLUETOOTH_FRAMEWORK_ASYNC
...
int do_light_ctrl(float light_perc)
{
    if (light_perc < 0.0f) light_perc = 0.0f;
    if (light_perc > 1.0f) light_perc = 1.0f;

    static char* arg_conn[26] = {"0", "0x0302", "hex", "70", "61", "69","72", "6c","69",
    "6e","6b","50","72","6f","66","69","6c","65","91","03","00","80","90",
    "1A","00","00"};
    uint16_t light_value = light_perc * 65535;
    unsigned int high = (light_value >> 8) & 0xFF;   // 高8位
    unsigned int low  = light_value & 0xFF;          // 低8位

    static char high_str[3];  // 2位 hex + '\0'
    static char low_str[3];
    sprintf(high_str, "%02X", high);
    sprintf(low_str, "%02X", low);

    arg_conn[21] = low_str;
    arg_conn[22] = high_str;
    gattc_write_cmd(g_lightCtrl_ins, 26, arg_conn);
    usleep(2000);
    return 0;
}

int bt_lightCtrl_main(int argc, char** argv)
{
    PRINT("btinstance_main CONFIG_LIBUV_EXTENSION**********");
    bttool_ins_init(NULL);
    bt_adapter_enable(g_lightCtrl_ins);
    while(1)
    {
        sleep(1);
    }
