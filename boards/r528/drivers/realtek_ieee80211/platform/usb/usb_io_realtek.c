...
{	
	...
	if (len > MAX_VENDOR_REQ_CMD_SIZE) {
		DBG_INFO("[%s] Buffer len error ,vendor request failed\n", __FUNCTION__);
		status = -22;
		goto exit;
	}
	...
	while (++vendorreq_times <= 10) {
		...
		if (status == len) {  /* Success this control transfer. */
			...
			DBG_INFO("reg 0x%x, usb %s %u fail, status:%d value=0x%x, vendorreq_times:%d\n"
...
{	
	...
	switch (pusbd->speed) {
	case 1:
		 DBG_INFO("Low Speed Case \n");
		 (speed) = RTW_USB_SPEED_1_1;
		 break;
	case 2:
		 DBG_INFO("full speed Case \n");
		 (speed) = RTW_USB_SPEED_1_1;
		 break;
	case 3:
		 DBG_INFO("high speed Case \n");
...
static int rtw_usb_get_in_ep_info(void *priv,unsigned char *pipe_num_arrays)
{
	...
	for (i = 0; i < (nr_endpoint); i++) {
		...
		if (phost_endp) {
			...
			DBG_INFO("bLength=%x\n", pendp_desc->bLength);
			DBG_INFO("bDescriptorType=%x\n", pendp_desc->bDescriptorType);
			DBG_INFO("bEndpointAddress=%x\n", pendp_desc->bEndpointAddress);
			DBG_INFO("wMaxPacketSize=%d\n", pendp_desc->wMaxPacketSize);
			DBG_INFO("bInterval=%x\n", pendp_desc->bInterval);
