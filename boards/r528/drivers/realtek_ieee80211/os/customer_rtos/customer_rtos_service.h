...
typedef irqstate_t		_irqL;
typedef void*			    _thread_hdl_;
typedef void			    thread_return;
typedef void*			    thread_context;

typedef struct { volatile int counter; } rtw_atomic_t;

#define ATOMIC_T rtw_atomic_t
...
#define printk				        wlinfo
//#define printk				printf

#ifndef __CC_ARM
#define DBG_INFO(fmt, args...) 		printk("RTL872X: " fmt, ## args)
#if defined(WLAN_INTF_DBG) && WLAN_INTF_DBG
#define DBG_TRACE(fmt, args...)		printk("RTL872X: [%s] " fmt, __FUNCTION__, ## args)
#else
#define DBG_TRACE(fmt, args...)
#endif
#else
#define DBG_INFO(...)		do {printk("RTL871X: ");printk(__VA_ARGS__);printk("\n\r");} while(0)
#if WLAN_INTF_DBG
#define DBG_TRACE(...)		do {printk("RTL871X: [%s] ", __FUNCTION__);printk(__VA_ARGS__);printk("\n\r");} while(0)
#else
#define DBG_TRACE(...)
#endif
#endif
...
#define ASSERT(x)			do { \
						if((x) == 0){\
							printk("RTL871X: Assert(" #x ") failed on line %d in file %s", __LINE__, __FILE__); \
						HALT();}\
					} while(0)
...
#define DBG_ASSERT(x, msg)		do { \
						if((x) == 0) \
							printk("RTL871X: %s, Assert(" #x ") failed on line %d in file %s", msg, __LINE__, __FILE__); \
					} while(0)
