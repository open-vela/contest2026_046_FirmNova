...
u32 rtw_systime_to_ms(u32 systime_t)
{
	if(osdep_service.rtw_systime_to_ms)
		return osdep_service.rtw_systime_to_ms(systime_t);
	else
...
u32 rtw_systime_to_sec(u32 systime_t)
{
	if(osdep_service.rtw_systime_to_sec)
		return osdep_service.rtw_systime_to_sec(systime_t);
	else
...
int rtw_retrieve_from_file(const char *path, u8 *buf, u32 sz)
{
  ...
  if (path && buf) {
    ...
    {
      DBG_INFO("Can't open firmware, %s", strerror(errno));
      return -1;
    }

  	if (stat(path, &st) < 0)
    {
      DBG_INFO("Can't access firmware %s, %s", path,
                  strerror(errno));
      close(fd);
      return -1;
    }
	...
	ret = read(fd, buf, fwsize);
  	if (ret != (ssize_t) fwsize)
    {
      DBG_INFO("Read FW %s error, %s", path, strerror(errno));
      close(fd);
      return -1;
    }
...
int rtw_get_fw_file_path_from_os(u8 * filepath,u8 cut_version, u8 wowlan)
{
	if(wowlan)
	{
		DBG_INFO("get wowlan fw patch fail,current not support wowlan\n");
		return _FALSE;
	}
	else
	{
		if(cut_version==0)
			rtw_memcpy(filepath,CCVWIFIFW_PATH,50); 
		else
			rtw_memcpy(filepath,WIFIFW_PATH,50); 
	}
