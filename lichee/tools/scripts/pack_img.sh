...
function merge_rivotek_res()
{
	local dst_dir="$1"
	local src_dir="${PACK_TOPDIR}/../rivotek/res"
	...
	pack_info "merge rivotek resources: ${src_dir} -> ${dst_dir}"
	mkdir -p "${dst_dir}"
...
{
	local part name size downloadfile dir_name merged_dir
	...
	do
		...
		merged_dir="${ROOT_DIR}/${PACK_PROJECT_PATH}/image/.${name}_merged"
		rm -rf "${merged_dir}"
		mkdir -p "${merged_dir}"
		cp -a "${dir_name}/." "${merged_dir}/"
		merge_rivotek_res "${merged_dir}"
		echo "copy ap.fex to ${merged_dir}/ap"
		mkdir -p ${merged_dir}/ap
		cp -rf ${PACK_TOPDIR}/board/${PACK_PROJECT_PATH}/configs/ap.fex ${merged_dir}/ap/
		name="${merged_dir}"
		echo "The filesystem path: ${name}"
