...
int _BlockCheck(unsigned short nDieNum, unsigned short nBlkNum, const char *func, unsigned long line);
#define BlockCheck(_nDieNum, _nBlkNum) _BlockCheck(_nDieNum, _nBlkNum, __func__, __LINE__)
...
int _BlockCheck(unsigned short nDieNum, unsigned short nBlkNum, const char *func, unsigned long line)
{
    ...
    if (ret)
        NFTL_ERR("%s:%lu return %d to %s:%lu\n", __func__, __LINE__, ret, func, line);
